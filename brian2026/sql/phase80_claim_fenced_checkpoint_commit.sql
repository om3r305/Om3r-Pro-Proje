-- Brian Phase 80 DRAFT SQL: claim-fenced authoritative checkpoint commit.
--
...
-- into an official migration with `supabase migration new`, then rerun through
-- the full real-Postgres suite before deployment.
--
-- Phase79 makes STARTED the durable point-of-no-return. Phase80 carries the
-- independent Phase77 worker claim fence all the way to the authoritative
-- Phase70 runtime checkpoint commit. A worker may do replay-safe paper work in
-- memory, but it cannot make that work authoritative after its claim expired or
-- was taken over by another worker.
