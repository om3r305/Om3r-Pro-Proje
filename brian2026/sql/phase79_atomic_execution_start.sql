-- Brian Phase 79 DRAFT SQL: atomic execution-start point-of-no-return.
--
...
-- into a migration created by `supabase migration new`, then re-run through
-- the same Postgres CI before deployment.
--
-- Phase78 already owns the pre/post execution risk decision semantics. Phase79
-- does not duplicate that policy. Instead it calls the Phase78 kill-switch
-- inside the SAME runtime advisory-lock transaction and records an immutable
-- STARTED boundary only when Phase78 says execution may proceed/resume.
--
-- Consequence:
--   * risk can veto until STARTED commits;
--   * after STARTED, later risk deterioration is post-start and must be handled
--     as CANCEL_REQUESTED/recovery, never as a fabricated rollback.
