...
declare r record;
begin
  for r in select jobid from cron.job where jobname='brian-official-primary-eye-v2-5m' loop
...
select cron.schedule(
  ...
  select net.http_post(
    url := (select decrypted_secret || '/functions/v1/brian-official-primary-eye-v2' from vault.decrypted_secrets where name='brian_project_url' limit 1),
