...
-- Bound the ALPHA audit candidate pool before expensive terminal-resolvability checks.
-- SHADOW MEASUREMENT ONLY. No live execution or capital path changes.
...
as $function$
  with cfg as materialized (
    select
      ...
      greatest(
        60,
        least(240, greatest(1, least(coalesce(p_limit, 120), 500)) * 2)
      )::int as pool,
      p_now - least(
        greatest(coalesce(p_lookback, interval '6 hours'), interval '2 hours'),
        interval '36 hours'
      ) as oldest
  ),

  recent_seed as materialized (
    ...
    limit (select pool from cfg)
  ),

  oldest_seed as materialized (
    select d.decision_id
    from public.brian_alpha_decisions d, cfg
    where d.observed_at >= cfg.oldest
      and d.observed_at <= p_now - interval '5 minutes'
  ...
  h60_seed as materialized (
    select d.decision_id
    from public.brian_alpha_decisions d, cfg
    where d.observed_at >= cfg.oldest
      and d.observed_at <= p_now - interval '60 minutes'
      and not exists (
        select 1
  ...
  h15_seed as materialized (
    select d.decision_id
    from public.brian_alpha_decisions d, cfg
    where d.observed_at >= cfg.oldest
      and d.observed_at <= p_now - interval '15 minutes'
      and not exists (
        select 1
        from public.brian_alpha_decision_outcomes o
        where o.decision_id=d.decision_id
  ...
  seed_ids as materialized (
    ...
    from (
      select decision_id from recent_seed
      union all
      select decision_id from oldest_seed
      union all
      select decision_id from h60_seed
      union all
      select decision_id from h15_seed
      union all
      select decision_id from h5_seed
    ) s
  ),
  ...
  eligible as materialized (
    select
      ...
      (
        ...
        and (
          ...
          or exists (
            select 1 from public.brian_alpha_decisions px
          ...
          or exists (
            select 1 from public.brian_intrabar_reaction_events ix
    ...
    from seed_ids s
    join public.brian_alpha_decisions d on d.decision_id=s.decision_id
  ),
  ...
  q60 as materialized (
    ...
    where e.missing_3600
      and not exists (select 1 from recent r where r.decision_id=e.decision_id)
  ...
  from picked p
  join public.brian_alpha_decisions d on d.decision_id=p.decision_id
...
'Service-role-only bounded ALPHA audit queue v2. Candidate selection is index-bounded before terminal resolvability checks, preserving recent plus 5m/15m/60m lane quotas without scanning the full lookback hot window. Shadow measurement only.';
