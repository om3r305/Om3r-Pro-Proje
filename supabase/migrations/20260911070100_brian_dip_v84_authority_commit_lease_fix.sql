...
begin
  ...
  select * into latest from public.brian_dip_v84_session_events order by requested_at desc,event_id desc limit 1;
  if not found or latest.session_id is distinct from p_session_id then raise exception 'V84_STALE_SESSION'; end if;
  if latest.event_kind='PAUSE' then
    select * into start_row from public.brian_dip_v84_session_events where session_id=p_session_id and event_kind='START' order by requested_at,event_id limit 1;
