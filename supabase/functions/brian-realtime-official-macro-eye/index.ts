...
const COLLECTOR_ID = "brian-realtime-official-macro-eye";
const RUNTIME_VERSION = "brian.realtime-official-macro-eye.v1";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const unauthorized = message.includes("UNAUTHORIZED_INTERNAL");
  ...
  try {
    const lease = await withCollectorLease(supabase, COLLECTOR_ID, LEASE_SECONDS, async () => {
      ...
      return json({ status, runtime:RUNTIME_VERSION, feeds_ok: FEEDS.length - degraded.length, feeds_failed: degraded.length, observed_items: observed, intel_events: events.length, recent_macro_observations: observations.length, degraded_sources: degraded, direction_inference: false, shadow_only: true, live_execution: false });
    });
