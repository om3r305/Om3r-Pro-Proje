...
Deno.serve(async(req:Request)=>{
  ...
  try{
    const [controlQ,runsQ,eventsQ,requestsQ,hypQ,artifactQ,sourcesQ,assessQ]=await Promise.all([
      ...
      db.from("brian_world_source_candidates").select("candidate_id,source_id,discovered_at,canonical_uri,provider,source_kind,authority_class,access_mode,stage,freshness_seconds,corroboration_required,manipulation_risk,rationale,metadata,evidence_class,shadow_only,live_execution").order("discovered_at",{ascending:false}).limit(320),
      db.from("brian_world_source_assessments").select("assessment_id,source_id,assessed_at,authority_score,freshness_score,manipulation_penalty,corroboration_penalty,access_penalty,trust_score,eligible_for_research,eligible_for_decision_evidence,reasons,metadata,evidence_class,shadow_only,live_execution").order("assessed_at",{ascending:false}).limit(640),
    ]);
    if(controlQ.error||!controlQ.data)throw new Error(`control:${controlQ.error?.message??"missing"}`);
    for(const [name,q] of [["runs",runsQ],["events",eventsQ],["requests",requestsQ],["hypotheses",hypQ],["artifacts",artifactQ],["sources",sourcesQ],["assessments",assessQ]] as const) if(q.error)throw new Error(`${name}:${q.error.message}`);
    ...
    const runRequestIds=new Set(runs.map((r:any)=>String(r.request_id)));
    ...
    const eligibleQueue=[...newestByHypothesis.values()].filter((r:any)=>!runRequestIds.has(String(r.request_id))&&r.required_human_review===true&&r.shadow_only===true&&r.live_execution===false&&r.autonomous_apply_allowed===false).sort((a:any,b:any)=>priority(b)-priority(a)||compareNewest(a,b));
    ...
    const sourceIds=new Set(sourceLibrary.map((s:any)=>String(s.source_id)));
    const sourceUris=sourceLibrary.map((s:any)=>String(s.canonical_uri??"")).filter(Boolean);
    ...
    function lineage(request:any,hyp:any){
      ...
      const directIds=[...sourceIds].filter(id=>all.some(r=>r.includes(id))).slice(0,20);
      const directUris=sourceUris.filter(uri=>all.some(r=>r.includes(uri))).slice(0,20);
      const matched=[...new Map([...directIds.map(id=>[id,sourceMap.get(id)]),...sourceLibrary.filter((s:any)=>directUris.includes(String(s.canonical_uri))).map((s:any)=>[String(s.source_id),s])]).values()].filter(Boolean);
      const classes={collector:all.filter(r=>r.startsWith("collector:")),outcome:all.filter(r=>r.includes("outcome")||r.includes("decision_outcomes")),world_source:all.filter(r=>r.includes("world_source")||r.startsWith("source:")),engineering:all.filter(r=>r.includes("review")||r.includes("compile")||r.includes("candidate")||r.includes("engineering")),other:all.filter(r=>!r.startsWith("collector:")&&!r.includes("outcome")&&!r.includes("world_source")&&!r.startsWith("source:")&&!r.includes("review")&&!r.includes("compile")&&!r.includes("candidate")&&!r.includes("engineering"))};
    ...
    const eligibleQueueView=eligibleQueue.map((request:any)=>{const hyp=latestHyp.get(String(request.hypothesis_id));return {...request,hypothesis:hyp??null,source_lineage:lineage(request,hyp)};});
    ...
    const trustFloor=numberOr(controlMeta.world_source_trust_floor,0.72);
    const worldReady=sourceLibrary.filter((s:any)=>s.assessment?.eligible_for_research===true&&numberOr(s.assessment?.trust_score,0)>=trustFloor&&s.authority_class==="OFFICIAL_PRIMARY"&&s.access_mode==="PUBLIC_NO_KEY"&&!['REJECTED','RETIRED','ARCHIVED'].includes(String(s.stage)));
    const worldHypotheses=[...latestHyp.values()].filter((h:any)=>typeof h?.metadata?.world_source_id==="string");
    const worldQueue=eligibleQueueView.filter((r:any)=>r.source_lineage?.direct_world_source_count>0||typeof r.hypothesis?.metadata?.world_source_id==="string");
    const worldRuns=enrichedRuns.filter((r:any)=>r.source_lineage?.direct_world_source_count>0||typeof r.hypothesis?.metadata?.world_source_id==="string");
    const now=Date.now(),windowStart=now-24*3600_000;
    const autonomousClaims24h=runs.filter((r:any)=>timeMs(r.created_at)>=windowStart&&String(r.metadata?.claim_mode??"")==="AUTONOMOUS").length;
    ...
    const summary={
      ...
      source_library_research_eligible:sourceLibrary.filter((s:any)=>s.assessment?.eligible_for_research===true).length,
