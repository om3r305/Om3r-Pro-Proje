...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const [controlQ,runsQ,claimedRequestQ,budgetRunsQ,reviewPassQ,reviewPhaseQ,eventsQ,requestsQ,hypQ,artifactQ,sourcesQ,assessQ]=await Promise.all([
      ...
      db.from("brian_world_source_latest").select("candidate_id,source_id,discovered_at,canonical_uri,provider,authority_class,access_mode,stage").order("discovered_at",{ascending:false}).limit(2000),
      db.from("brian_world_source_assessment_latest").select("source_id,assessed_at,trust_score,eligible_for_research").order("assessed_at",{ascending:false}).limit(2000),
    ]);
