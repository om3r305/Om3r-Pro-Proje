...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const [controlQ,runsQ,claimedRequestQ,budgetRunsQ,reviewPassQ,reviewPhaseQ,eventsQ,requestsQ,hypQ,artifactQ,sourcesQ,assessQ]=await Promise.all([
      ...
      db.from("brian_world_source_candidates").select("candidate_id,source_id,discovered_at,canonical_uri,provider,source_kind,authority_class,access_mode,stage,freshness_seconds,corroboration_required,manipulation_risk,rationale,metadata,evidence_class,shadow_only,live_execution").order("discovered_at",{ascending:false}).limit(1000),
      db.from("brian_world_source_assessments").select("assessment_id,source_id,assessed_at,authority_score,freshness_score,manipulation_penalty,corroboration_penalty,access_penalty,trust_score,eligible_for_research,eligible_for_decision_evidence,reasons,metadata,evidence_class,shadow_only,live_execution").order("assessed_at",{ascending:false}).limit(1000),
    ]);
