...
Deno.serve(async(req:Request)=>{
  ...
  try{
    const [controlQ,runsQ,eventsQ,requestsQ,hypQ,artifactQ,sourcesQ,assessQ]=await Promise.all([
      ...
      db.from("brian_world_source_candidates").select("candidate_id,source_id,discovered_at,canonical_uri,provider,source_kind,authority_class,access_mode,stage,freshness_seconds,corroboration_required,manipulation_risk,rationale,metadata,evidence_class,shadow_only,live_execution").order("discovered_at",{ascending:false}).limit(160),
      db.from("brian_world_source_assessments").select("assessment_id,source_id,assessed_at,authority_score,freshness_score,manipulation_penalty,corroboration_penalty,access_penalty,trust_score,eligible_for_research,eligible_for_decision_evidence,reasons,metadata,evidence_class,shadow_only,live_execution").order("assessed_at",{ascending:false}).limit(320),
    ]);
    ...
    const sourceLibrary=sources.map((s:any)=>({...s,assessment:latestAssess.get(String(s.source_id))??null}));
    ...
    return out({status:"ONLINE",observed_at:new Date().toISOString(),control:controlQ.data,summary,pipeline:PIPELINE,current_run:active[0]??null,approval_run:approvals[0]??null,approval_runs:approvals,recent_runs:enrichedRuns,eligible_queue:eligibleQueue.slice(0,20),source_library:sourceLibrary.slice(0,80),truth:{self_coding_enabled:controlQ.data.autonomous_claim_enabled===true,end_to_end_proven:summary.completed_runs>0||summary.waiting_human_approval>0,human_approval_waiting:summary.waiting_human_approval,world_library_directly_linked_to_focus:(approvals[0]??active[0]??enrichedRuns[0])?.source_lineage?.direct_world_source_count>0},shadow_only:true,live_execution:false,dip_isolated:true},200,origin);
