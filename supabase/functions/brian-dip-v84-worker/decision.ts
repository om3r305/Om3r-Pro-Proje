...
type SetupCandidate={setup:Setup;direction:TradeDirection;trigger:Struct;priority:number;provisionalStop:number|null;reason:string};
export type CandidateTelemetry={
  rank:number;selected:boolean;setup:Setup;direction:TradeDirection;trigger_tf:string;reason:string;base_priority:number;
...
function recentReversal(m:Market,s1:Struct,s5:Struct):SetupCandidate|null{
  const rows=m.bars["1m"].slice(-8);if(rows.length<8)return null;const last=rows.at(-1)!,prev=rows.at(-2)!,prev2=rows.at(-3)!,recentLow=Math.min(...rows.slice(-7).map(x=>x.l)),recentHigh=Math.max(...rows.slice(-7).map(x=>x.h)),fast=m.flowFast.ofi,slow=m.flowSlow.ofi,improve=fast-slow;
  ...
  if(upMomentum&&upFlow&&!upConflict&&upDistance>=minMove)return{setup:"EARLY_REVERSAL",direction:"UP",trigger:s1,priority:6,provisionalStop:recentLow,reason:`1m momentum + flow recovery ${fast.toFixed(2)}/${slow.toFixed(2)}`};
  if(downMomentum&&downFlow&&!downConflict&&downDistance>=minMove)return{setup:"EARLY_REVERSAL",direction:"DOWN",trigger:s1,priority:6,provisionalStop:recentHigh,reason:`1m momentum + flow deterioration ${fast.toFixed(2)}/${slow.toFixed(2)}`};
  return null;
}
...
function collectCandidates(m:Market,s1:Struct,s5:Struct,s15:Struct):{ranked:SetupCandidate[];telemetry:CandidateTelemetry[]}{
  ...
  for(const s of [s1,s5]){
    if(s.sweep)raw.push({setup:"SWEEP_RECLAIM",direction:s.sweep==="BULL"?"UP":"DOWN",trigger:s,priority:s.tf==="5m"?8:7,provisionalStop:null,reason:`${s.tf} ${s.sweep} sweep`});
    if(s.failedBreak)raw.push({setup:"FAILED_BREAK",direction:s.failedBreak==="BULL"?"UP":"DOWN",trigger:s,priority:s.tf==="5m"?8:7,provisionalStop:null,reason:`${s.tf} ${s.failedBreak} failed break`});
  }
  if(s5.bos){const p=s5.bos==="UP"?s5.lastHigh:s5.lastLow;if(p&&Math.abs(m.book.mid-p.p)<=s5.atr*.45)raw.push({setup:"BOS_RETEST",direction:s5.bos,trigger:s5,priority:7,provisionalStop:null,reason:"5m BOS retest"});}
...
function stopForCandidate(chosen:SetupCandidate|null,direction:Direction){
  ...
  if(chosen.setup==="EARLY_REVERSAL")return{price:chosen.provisionalStop,source:"RECENT_7_BAR_EXTREME",structureId:chosen.trigger.fingerprint};
...
function firstVeto(veto:string[]):{first:string|null;stage:string|null}{for(const v of veto){const root=v.split(":")[0];return{first:v,stage:STAGE[root]??"OTHER"};}return{first:null,stage:null};}
...
export async function candidate(m:Market,sessionId:string,rt:Runtime,cfg:J,tradeNotional:number,at:number,cal:ExecutionCalibration):Promise<CandidateResult>{
  ...
  const immutableIdentity=setup&&isTradeDirection(direction)?[setup,direction,trigger?.tf,stop.structureId,stop.source,l1?.level_id??"NO_L1",combinedFp].join("|"):"NO_SIGNAL";
  const episode=await hash([SYMBOL,immutableIdentity].join("|")),occurrence=await hash([sessionId,RELEASE_ID,DECISION_REVISION,episode,signalAt,immutableIdentity].join("|"));
  const veto:string[]=[];
