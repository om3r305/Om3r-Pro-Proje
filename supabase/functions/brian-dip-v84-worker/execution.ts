...
function exitFromBarrier(pos:Position,barrier:number):number{
  ...
  // Barrier/reference price -> executable-side conversion once + slippage once.
...
function executionBarrier(pos:Position,resolution:Resolution):number{
  // Ordered aggTrade evidence proves WHICH barrier happened first; it does not silently
  // redefine the frozen execution plan with the first observed trade print. Keeping the
  // barrier separate prevents a trade trigger price from being treated as a new stop/TP.
