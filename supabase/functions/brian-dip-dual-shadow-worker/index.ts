...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    return Response.json(lease.contended?{status:"WAIT_LEASE",worker_version:"dip-v8.3-rootfix-20260908.4"}:lease.value,{headers:{"cache-control":"no-store"}});
  }catch(e){const message=e instanceof Error?e.message:String(e);console.error("dip-v83-dual",message);return Response.json({status:"FAILED_CLOSED",error:message,worker_version:"dip-v8.3-rootfix-20260908.4",policy_version:POLICY_VERSION,shadow_only:true,live_execution:false},{status:message.includes("UNAUTHORIZED")?401:500,headers:{"cache-control":"no-store"}});}
});
