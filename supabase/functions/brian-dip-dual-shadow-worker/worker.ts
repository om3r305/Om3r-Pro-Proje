...
import {
  calibrate, closePosition, ENGINE_VERSION, evaluatePath, hash, type J,
...
export async function readRuntime(db:SupabaseClient,sessionId:string):Promise<{runtime:Runtime;state_version:number}>{
  const q=await db.from("brian_dip_v8_runtime").select("runtime,state_version").eq("session_id",sessionId).maybeSingle();
