...
const VERSION = "brian.realtime-orchestrator.v3";
...
async function call(name:string,url:string,key:string,timeoutMs:number){
  ...
  try{
    ...
    return {name,ok:logicalOk,http_status:r.status,target_status:targetStatus,elapsed_ms:Date.now()-started,body};
  }catch(e){
...
async function marketLane(key:string,minute:number){
  ...
  if(minute%5===0){
    ...
    if(universe.ok){
      ...
      if(sensor.ok){
        if(minute%10===0) results.push(await call("derivatives",ENDPOINTS.derivatives,key,50000));
  ...
  if(minute%60===7) results.push(await call("fx",ENDPOINTS.fx,key,50000));

  results.push(await call("intrabar",ENDPOINTS.intrabar,key,50000));

  if(minute%2===0) results.push(await call("alpha",ENDPOINTS.alpha,key,50000));
  return results;
}
