...
const V="brian.realtime-alpha-recheck.v1", E="PROSPECTIVE_DEVELOPMENT_SHADOW";
...
try{
  const br=await fetch("https://qbcjuxhvhwagvqbjyemo.supabase.co/functions/v1/brian-realtime-alpha-bridge",{
    method:"POST",
    headers:{"content-type":"application/json","x-brian-internal-key":internalKey},
    body:JSON.stringify({costs:costRow?[costRow]:[],decisions:[row],shadow_only:true,live_execution:false}),
    signal:AbortSignal.timeout(12000)
  });
  if(!br.ok)throw Error("CORE_ALPHA_BRIDGE_"+br.status+":"+(await br.text()).slice(0,500));
}catch(e){console.error("core alpha bridge",String(e))}
