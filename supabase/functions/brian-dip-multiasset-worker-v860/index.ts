...
const ENGINE_VERSION='V8.9.8';
const POLICY_VERSION='dip-v898-calibrated-scout-wave-memory-20260921.1';
const RISK_ENGINE='V898_CALIBRATED_SCOUT_WAVE_MEMORY';
...
function evaluate(c:Candidate,m:Market,mode:any,breadth:Breadth):Eval{
  ...
  const forecastU=forecastUtility(m),brainUtility=decisionUtility({score,opp,explosionScore,net},m),
    defensiveScoutEvidence=mode.name!=='DEFENSIVE'||(forecastU>=.80&&brainUtility>=.80&&explosionScore>=.68&&f.continuation>=.68&&f.expected_30m_bps>=20),
    failed=Object.entries(gates).filter(([,v])=>!v).map(([k])=>k),allGates=failed.length===0,
    ...
    scoutRaw=!mode.frozen&&!coreRaw&&!winnerRaw&&!hunterRaw&&defensiveScoutEvidence&&(
...
function reentryContext(guard:any,m:Market,ev:Eval,now:number){
  ...
  if(pnl>.01&&(reason==='PEAK_PROFIT_LOCK'||reason==='PROFIT_RATCHET')){
    const f=ev.forecast,exitBrainStrong=Boolean(guard.exit_brain_strong)||num(guard.exit_continuation)>=.72,
      safe=Boolean(ev.gates.spread&&ev.gates.day_move&&ev.gates.forecast_consistency&&ev.gates.regime_guard&&ev.gates.shock_memory&&ev.gates.long_extension),
      priceAdvance=exitPrice>0?(m.mid/exitPrice-1)*10000:9999,
      explosiveContinuation=exitBrainStrong&&safe&&f.continuation>=.84&&f.expected_5m_bps>=35&&f.expected_15m_bps>=70&&f.expected_30m_bps>=100&&
        f.ret1_bps>=8&&f.ret1_bps<=120&&f.ret3_bps>=0&&f.ret3_bps<=180&&priceAdvance>=-25&&priceAdvance<=180;
    if(age>=20_000&&age<=3*60_000&&explosiveContinuation)return{active:true,eligible:true,type:'RUNNER_RECLAIM',reason:'V884_RUNNER_RECLAIM',guard};
    if(age<3*60_000)return{active:true,eligible:false,type:'RUNNER_RECLAIM',reason:age<20_000?'WAIT_RUNNER_RECLAIM_CONFIRM':'WAIT_RUNNER_RECLAIM_STRENGTH',guard};
  }
...
function pulseOk(p:EntryPulse,reentryType:string,grossFrac:number,style:string){
  if(reentryType==='RUNNER_RECLAIM')return p.buy_ratio>=.56&&p.ret5_bps>=0&&p.ret15_bps>=0&&p.ret5_bps<=70&&p.ret15_bps<=120;
...
async function loadArenaState(sql:any,main:State){
  const rows=await sql`select * from public.brian_dip_multiasset_state where engine_id=${ARENA_ID} limit 1`;
  if(rows.length)return rows[0] as any;
  const started=new Date(),runUntil=new Date(main.run_until),source=String((main as any).source_session_id||'')||null,
    scan={status:'RUNNING',mode:'AGGRESSIVE_ARENA',engine_version:ENGINE_VERSION,policy_version:POLICY_VERSION,source:'SHARED_MAIN_SCAN',heartbeat_at:iso(),recent_events:[],no_extra_full_scan:true,no_evaluation_writes:true};
  const out=await sql`insert into public.brian_dip_multiasset_state
    (engine_id,source_session_id,started_at,run_until,starting_equity,cash,realized_pnl,trade_count,win_count,loss_count,positions,cooldowns,last_scan,last_eval_minute,updated_at,enabled,shadow_only,live_execution)
    values (${ARENA_ID},${source},${started},${runUntil},1000,1000,0,0,0,0,${sql.json({})},${sql.json({})},${sql.json(scan)},null,now(),true,true,false)
    returning *`;
  return out[0] as any;
}
function arenaVote(m:Market){
  const f=m.forecast;
  return f.expected_5m_bps*.16+f.expected_15m_bps*.27+f.expected_30m_bps*.31+f.expected_60m_bps*.10;
}
function arenaEquity(cash:number,positions:Record<string,any>,markets:Map<string,Market>){
  let e=cash;
  for(const p of Object.values(positions)){const m=markets.get(String(p.symbol));const mark=m?.bid||num(p.entry);e+=num(p.qty)*mark*(1-FEE_BPS/10000);}
  return e;
}
async function insertArenaEvent(sql:any,row:any){
  await sql`insert into public.brian_dip_multiasset_events (engine_id,observed_at,symbol,action,price,qty,notional,pnl,reason,metadata)
    values (${ARENA_ID},${new Date(row.observed_at)},${row.symbol},${row.action},${row.price},${row.qty},${row.notional},${row.pnl},${row.reason},${sql.json(row.metadata||{})})`;
}
async function processArena(sql:any,arena:any,pool:any[],markets:Map<string,Market>,breadth:Breadth,now:number){
  const positions:{[k:string]:any}={...(arena.positions||{})},cooldowns:Record<string,any>={...(arena.cooldowns||{})};
  let cash=num(arena.cash,1000),realized=num(arena.realized_pnl),trades=num(arena.trade_count),wins=num(arena.win_count),losses=num(arena.loss_count);
  const previousEvents=Array.isArray(arena.last_scan?.recent_events)?arena.last_scan.recent_events:[],recentEvents:any[]=[...previousEvents].slice(0,20),actions:any[]=[];
  const close=async(symbol:string,reason:string)=>{
    const p=positions[symbol],m=markets.get(symbol);if(!p||!m)return false;
    const slip=Math.max(2,m.spreadBps/2),exit=m.bid*(1-slip/10000),gross=num(p.qty)*exit,fee=gross*FEE_BPS/10000,pnl=gross-fee-num(p.cost_basis);
    cash+=gross-fee;realized+=pnl;trades++;if(pnl>.01)wins++;else if(pnl<-.01)losses++;
    cooldowns[symbol]={exit_at:now,exit_price:exit,reason,pnl,post_exit_low:exit,post_exit_high:exit,weak_seen:false,exit_forecast_utility:forecastUtility(m),exit_vote:arenaVote(m),exit_bar_close:num(m.lastBarCloseTime),prior_entry:num(p.entry),wave_reset_consumed:false};
    const ev={observed_at:iso(),symbol,action:'SELL',price:exit,qty:num(p.qty),notional:gross,pnl,reason,metadata:{
      arena:true,arena_mode:'TOP2_CONCENTRATED',slot_kind:p.slot_kind||'REGULAR',entry:num(p.entry),entry_brain_utility:num(p.entry_brain_utility),
      current_forecast_utility:forecastUtility(m),forecast_vote_bps:arenaVote(m),max_price:num(p.max_price),allocation_fraction:num(p.allocation_fraction),
      engine_version:ENGINE_VERSION,policy_version:POLICY_VERSION,shadow_only:true,live_execution:false}};
    delete positions[symbol];await insertArenaEvent(sql,ev);actions.push(ev);recentEvents.unshift(ev);recentEvents.splice(20);
    return true;
  };

  for(const [symbol,p] of Object.entries(positions)){
    const m=markets.get(symbol);if(!m)continue;
    const fu=forecastUtility(m),vote=arenaVote(m),entryU=num(p.entry_forecast_utility,fu),peakU=Math.max(num(p.peak_forecast_utility,entryU),fu),
      barKey=num(m.lastBarCloseTime),newThesisBar=barKey>num(p.last_thesis_bar_close),
      soft=(entryU-fu>.11||peakU-fu>.17)&&vote<0,
      streak=newThesisBar?(soft?Math.min(3,num(p.thesis_decay_streak)+1):Math.max(0,num(p.thesis_decay_streak)-1)):num(p.thesis_decay_streak),
      age=Math.max(0,now-Date.parse(String(p.opened_at||iso()))),
      noMfe=num(p.max_price,p.entry)<=num(p.entry)*1.0010,
      rapidFailure=age>=55_000&&noMfe&&fu<=entryU-.22&&(vote<5||m.forecast.expected_30m_bps<0||m.forecast.continuation<.50),
      disaster=m.bid<=num(p.disaster_stop,num(p.entry)*.95),hard=fu<.28&&vote<0,thesisBreak=hard||streak>=2;
    p.last_forecast_utility=fu;p.peak_forecast_utility=peakU;p.thesis_decay_streak=streak;p.max_price=Math.max(num(p.max_price,p.entry),m.bid);p.last_mark=m.bid;p.last_vote=vote;
    if(newThesisBar)p.last_thesis_bar_close=barKey;
    if(disaster)await close(symbol,'ARENA_DISASTER_STOP');
    else if(rapidFailure)await close(symbol,'ARENA_RAPID_THESIS_FAILURE');
    else if(thesisBreak)await close(symbol,'ARENA_THESIS_BREAK');
  }

  const prevCandidateMemory=((arena.last_scan?.candidate_memory||{}) as Record<string,any>),candidateMemory:Record<string,any>={};
  for(const [sym,mem] of Object.entries(prevCandidateMemory)){if(now-num((mem as any)?.last_seen)<180_000)candidateMemory[sym]=mem;}
  const candidateStreaks:Record<string,number>={},
    exValues=pool.map(x=>num(x.ev?.explosionScore)).filter(x=>x>0).sort((a,b)=>a-b),
    p90=exValues.length?exValues[Math.max(0,Math.min(exValues.length-1,Math.floor((exValues.length-1)*.90)))]:.70,
    adaptiveExplosion=Math.min(.84,Math.max(breadth.riskOff?.74:.70,p90+.035));
  const allCandidates=pool.filter(x=>!positions[x.c.symbol]).map(x=>{
    const f=x.ev.forecast,g=x.ev.gates||{},barKey=num(x.m?.lastBarCloseTime),guard:any=cooldowns[x.c.symbol],
      safe=Boolean(g.spread&&g.day_move&&g.forecast_consistency&&g.horizon_alignment&&g.regime_guard&&g.shock_memory&&g.post_pump_reset&&g.long_extension&&g.economics),
      absoluteQuality=Boolean(x.ev.explosionScore>=.74&&x.ev.brainUtility>=.82&&x.ev.forecastUtility>=.76&&x.ev.opp>=.70&&f.continuation>=.66&&x.ev.net>=35),
      baseQualified=safe&&absoluteQuality&&x.ev.explosionScore>=adaptiveExplosion&&f.expected_15m_bps>=22&&f.expected_30m_bps>=35,
      eliteException=safe&&x.ev.explosionScore>=.82&&x.ev.brainUtility>=.87&&x.ev.forecastUtility>=.84&&x.ev.opp>=.76&&f.continuation>=.74&&x.ev.net>=65&&f.expected_30m_bps>=75,
      exitPrice=num(guard?.exit_price),exitAge=now-num(guard?.exit_at),resetFrac=Math.max(.004,Math.min(.018,x.m.atrPct*.38/100)),
      reclaimFrac=Math.max(.0025,Math.min(.010,x.m.atrPct*.22/100)),
      postLow=guard?Math.min(num(guard.post_exit_low,exitPrice||x.m.mid),x.m.mid):x.m.mid,
      weakSeen=Boolean(guard&&(guard.weak_seen===true||x.ev.forecastUtility<.52||f.expected_30m_bps<0||f.continuation<.48)),
      lossExit=Boolean(guard&&(num(guard.pnl)<-.01||['ARENA_THESIS_BREAK','ARENA_RAPID_THESIS_FAILURE','ARENA_DISASTER_STOP'].includes(String(guard.reason||'')))),
      priceResetSeen=Boolean(!guard||exitPrice<=0||postLow<=exitPrice*(1-resetFrac)),
      resetSeen=Boolean(!guard||exitPrice<=0||(lossExit?priceResetSeen:(priceResetSeen||weakSeen))),
      reclaimed=Boolean(!guard||exitPrice<=0||x.m.mid>=postLow*(1+reclaimFrac)),
      newWave=Boolean(!guard||exitAge>120*60_000||(resetSeen&&reclaimed&&barKey>num(guard.exit_bar_close))),
      qualified=Boolean((baseQualified||eliteException)&&newWave),
      prevMem=candidateMemory[x.c.symbol],prevFresh=prevMem&&now-num(prevMem.last_seen)<180_000,
      newEvidenceBar=Boolean(barKey>0&&barKey>num(prevMem?.last_evidence_bar_close)),
      prevWasWeak=Boolean(prevFresh&&(num(prevMem?.forecast_utility,.5)<.50||num(prevMem?.expected_15m_bps)<0||num(prevMem?.expected_30m_bps)<0||num(prevMem?.continuation,.5)<.50)),
      reboundTest=Boolean(qualified&&prevWasWeak),
      priorStreak=prevFresh?num(prevMem?.streak):0,
      streak=qualified?(newEvidenceBar?Math.min(4,priorStreak+1):priorStreak):0,
      arenaScore=clip(x.ev.explosionScore*.48+x.ev.brainUtility*.23+x.ev.forecastUtility*.11+x.ev.opp*.10+f.continuation*.08);
    if(guard){
      guard.post_exit_low=postLow;guard.post_exit_high=Math.max(num(guard.post_exit_high,exitPrice||x.m.mid),x.m.mid);guard.weak_seen=weakSeen;
      if(newWave&&qualified)guard.wave_reset_consumed=true;
    }
    candidateMemory[x.c.symbol]={streak,last_seen:now,last_evidence_bar_close:barKey,arena_score:arenaScore,explosion_score:x.ev.explosionScore,
      forecast_utility:x.ev.forecastUtility,expected_15m_bps:f.expected_15m_bps,expected_30m_bps:f.expected_30m_bps,continuation:f.continuation,
      qualified,rebound_test:reboundTest,new_wave:newWave,absolute_quality:absoluteQuality,same_wave_blocked:Boolean(guard&&!newWave)};
    candidateStreaks[x.c.symbol]=streak;
    return{...x,arenaScore,arenaQualified:qualified,arenaStreak:streak,arenaNewEvidenceBar:newEvidenceBar,arenaReboundTest:reboundTest,arenaNewWave:newWave,arenaAbsoluteQuality:absoluteQuality,arenaSameWaveBlocked:Boolean(guard&&!newWave)};
  }).sort((a,b)=>b.arenaScore-a.arenaScore);
  const candidates=allCandidates.filter(x=>x.arenaQualified&&x.arenaStreak>=2);

  const posScore=(p:any)=>{
    const m=markets.get(String(p.symbol));if(!m)return num(p.last_forecast_utility,.5);
    return clip(forecastUtility(m)*.78+num(p.entry_explosion_score,.5)*.22);
  };
  const posPnl=(p:any)=>{
    const m=markets.get(String(p.symbol));if(!m)return 0;
    const gross=num(p.qty)*m.bid,fee=gross*FEE_BPS/10000;return gross-fee-num(p.cost_basis);
  };

  const monster=candidates.find(x=>x.ev.explosionScore>=.86&&x.ev.brainUtility>=.86&&x.ev.forecastUtility>=.82&&x.ev.opp>=.78&&x.ev.forecast.continuation>=.78&&x.ev.net>=70&&x.ev.forecast.expected_15m_bps>=50&&x.ev.forecast.expected_30m_bps>=85);
  const ultraMonster=monster&&monster.ev.explosionScore>=.92&&monster.ev.brainUtility>=.90&&monster.ev.forecastUtility>=.88&&monster.ev.forecast.continuation>=.86&&monster.ev.forecast.expected_15m_bps>=80&&monster.ev.forecast.expected_30m_bps>=125;
  let monsterOverride=false;
  if(monster){
    const current=Object.values(positions),bestOpen=current.length?Math.max(...current.map(posScore)):0;
    const dominates=current.length===0||monster.arenaScore>=bestOpen+.08;
    if(dominates){
      const profitable=current.filter(p=>posPnl(p)>.01&&monster.arenaScore>=posScore(p)+.08).sort((a,b)=>posScore(a)-posScore(b));
      if(current.length>=2&&profitable.length){
        for(const p of profitable)await close(String(p.symbol),'ARENA_MONSTER_ROTATE');
      }
      const equity=arenaEquity(cash,positions,markets),freeFrac=equity>0?cash/equity:0;
      if(!positions[monster.c.symbol]&&cash>=MIN_SHADOW_NOTIONAL&&(Object.keys(positions).length<3||freeFrac>=.20)){
        const fullConcentration=Object.keys(positions).length===0;
        const monsterTarget=ultraMonster?.995:.85;
        const grossBudget=Math.min(cash*.995,equity*(fullConcentration?monsterTarget:Math.min(monsterTarget,freeFrac*.99)));
        if(grossBudget>=MIN_SHADOW_NOTIONAL){
          const m=monster.m,slip=Math.max(2,m.spreadBps/2),gross=grossBudget/(1+FEE_BPS/10000),entry=m.ask*(1+slip/10000),qty=gross/entry,fee=gross*FEE_BPS/10000,costBasis=gross+fee,
            stopGap=Math.max(.05,m.atrPct*2.5/100),p:any={symbol:monster.c.symbol,entry,qty,cost_basis:costBasis,opened_at:iso(),max_price:entry,disaster_stop:entry*(1-stopGap),
              entry_brain_utility:monster.ev.brainUtility,entry_forecast_utility:monster.ev.forecastUtility,last_forecast_utility:monster.ev.forecastUtility,peak_forecast_utility:monster.ev.forecastUtility,
              entry_explosion_score:monster.ev.explosionScore,allocation_fraction:gross/equity,slot_kind:'MONSTER',thesis_decay_streak:0};
          positions[p.symbol]=p;cash-=costBasis;monsterOverride=true;
          const ev={observed_at:iso(),symbol:p.symbol,action:'BUY',price:entry,qty,notional:gross,pnl:null,reason:'ARENA_MONSTER_ALLIN',metadata:{
            arena:true,arena_mode:'TOP2_CONCENTRATED',monster_override:true,arena_score:monster.arenaScore,brain_utility:monster.ev.brainUtility,forecast_utility:monster.ev.forecastUtility,
            explosion_score:monster.ev.explosionScore,allocation_fraction:p.allocation_fraction,engine_version:ENGINE_VERSION,policy_version:POLICY_VERSION,shadow_only:true,live_execution:false}};
          await insertArenaEvent(sql,ev);actions.push(ev);recentEvents.unshift(ev);recentEvents.splice(20);
        }
      }
    }
  }
  ...
  if(!monsterOverride){
    for(const x of candidates){
      const count=Object.keys(positions).length;if(count>=2)break;if(positions[x.c.symbol])continue;
      const equity=arenaEquity(cash,positions,markets);if(!(equity>0&&cash>=MIN_SHADOW_NOTIONAL))break;
      const exConv=clip((x.ev.explosionScore-.74)/.18),
        brainConv=clip((x.ev.brainUtility-.82)/.12),forecastConv=clip((x.ev.forecastUtility-.76)/.18),
        oppConv=clip((x.ev.opp-.70)/.16),contConv=clip((x.ev.forecast.continuation-.66)/.20),
        evidenceConv=clip(exConv*.58+brainConv*.14+forecastConv*.10+oppConv*.10+contConv*.08),
        firstFrac=x.ev.explosionScore>=.86&&x.ev.brainUtility>=.88?.65:x.ev.explosionScore>=.80?.45:.18+.22*evidenceConv,
        secondAllowed=count===0||x.arenaScore>=Math.max(.80,(candidates[0]?.arenaScore||0)-.06),
        secondFrac=x.ev.explosionScore>=.82?.38:.14+.18*evidenceConv,
        targetFrac=count===0?firstFrac:(secondAllowed?Math.min(.48,secondFrac,cash/equity*.995):0),
        budget=Math.min(cash*.995,equity*targetFrac);
      if(budget<MIN_SHADOW_NOTIONAL)continue;
      const m=x.m,slip=Math.max(2,m.spreadBps/2),gross=budget/(1+FEE_BPS/10000),entry=m.ask*(1+slip/10000),qty=gross/entry,fee=gross*FEE_BPS/10000,costBasis=gross+fee,
        stopGap=Math.max(.05,m.atrPct*2.5/100),p:any={symbol:x.c.symbol,entry,qty,cost_basis:costBasis,opened_at:iso(),max_price:entry,disaster_stop:entry*(1-stopGap),
          entry_brain_utility:x.ev.brainUtility,entry_forecast_utility:x.ev.forecastUtility,last_forecast_utility:x.ev.forecastUtility,peak_forecast_utility:x.ev.forecastUtility,
          entry_explosion_score:x.ev.explosionScore,allocation_fraction:gross/equity,slot_kind:'REGULAR',thesis_decay_streak:0,last_thesis_bar_close:num(m.lastBarCloseTime)};
      positions[p.symbol]=p;cash-=costBasis;
      const ev={observed_at:iso(),symbol:p.symbol,action:'BUY',price:entry,qty,notional:gross,pnl:null,reason:count===0?'ARENA_EXPLOSION_TOP1':'ARENA_EXPLOSION_TOP2',metadata:{
        arena:true,arena_mode:'TOP2_CONCENTRATED',arena_score:x.arenaScore,brain_utility:x.ev.brainUtility,
        forecast_utility:x.ev.forecastUtility,explosion_score:x.ev.explosionScore,opportunity_score:x.ev.opp,continuation:x.ev.forecast.continuation,
        evidence_conviction:evidenceConv,adaptive_explosion_threshold:adaptiveExplosion,confirmation_streak:x.arenaStreak,confirmation_bar_close:num(m.lastBarCloseTime),
        rebound_test:x.arenaReboundTest===true,allocation_fraction:p.allocation_fraction,engine_version:ENGINE_VERSION,policy_version:POLICY_VERSION,shadow_only:true,live_execution:false}};
      await insertArenaEvent(sql,ev);actions.push(ev);recentEvents.unshift(ev);recentEvents.splice(20);
    }
  }

  const eq=arenaEquity(cash,positions,markets),marks=Object.values(positions).map((p:any)=>{const m=markets.get(String(p.symbol)),mark=m?.bid||num(p.entry),gross=num(p.qty)*mark,fee=gross*FEE_BPS/10000;
    return{symbol:p.symbol,entry:num(p.entry),mark,qty:num(p.qty),notional:num(p.qty)*num(p.entry),allocation_fraction:num(p.allocation_fraction),slot_kind:p.slot_kind||'REGULAR',
      brain_utility:num(p.last_forecast_utility,p.entry_brain_utility),entry_brain_utility:num(p.entry_brain_utility),forecast_utility:num(p.last_forecast_utility),explosion_score:num(p.entry_explosion_score),
      unrealized_pnl:gross-fee-num(p.cost_basis),max_price:num(p.max_price),opened_at:p.opened_at};});
  const arenaWatch=allCandidates.slice(0,8).map(x=>({symbol:x.c.symbol,qualified:x.arenaQualified,confirm_streak:x.arenaStreak,new_evidence_bar:x.arenaNewEvidenceBar,rebound_test:x.arenaReboundTest,
    new_wave:x.arenaNewWave,absolute_quality:x.arenaAbsoluteQuality,same_wave_blocked:x.arenaSameWaveBlocked,arena_score:x.arenaScore,brain_utility:x.ev.brainUtility,forecast_utility:x.ev.forecastUtility,explosion_score:x.ev.explosionScore,opportunity_score:x.ev.opp,
    continuation:x.ev.forecast.continuation,forecast_net_bps:x.ev.net,expected_15m_bps:x.ev.forecast.expected_15m_bps,expected_30m_bps:x.ev.forecast.expected_30m_bps,last_bar_close_time:num(x.m?.lastBarCloseTime)}));
  const scan={status:'RUNNING',mode:'AGGRESSIVE_ARENA',engine_version:ENGINE_VERSION,policy_version:POLICY_VERSION,source:'SHARED_MAIN_SCAN',heartbeat_at:iso(),
    no_extra_full_scan:true,no_evaluation_writes:true,max_regular_positions:2,max_positions_with_monster:3,monster_override:true,selection_mode:'EXPLOSION_FIRST_ADAPTIVE',
    adaptive_explosion_threshold:adaptiveExplosion,explosion_p90:p90,candidate_streaks:candidateStreaks,candidate_memory:candidateMemory,confirmation_cycles:2,confirmation_requires_distinct_closed_bar:true,candidate_memory_ttl_seconds:180,breadth,absolute_quality_floor:true,same_wave_memory:true,forecast_horizon_calibration:true,regular_allocation:'~18-65% evidence-scaled; 45%+ requires stronger explosion',top2_only_if_qualified:true,monster_target:'85%',ultra_monster_target:'99.5%',
    equity:eq,open_positions:marks,arena_watch:arenaWatch,recent_events:recentEvents.slice(0,20),actions:actions.slice(0,8)};
  await sql`update public.brian_dip_multiasset_state set cash=${cash},realized_pnl=${realized},trade_count=${trades},win_count=${wins},loss_count=${losses},
    positions=${sql.json(positions)},cooldowns=${sql.json(cooldowns)},last_scan=${sql.json(scan)},updated_at=now(),enabled=true,shadow_only=true,live_execution=false,run_until=${new Date(arena.run_until)}
    where engine_id=${ARENA_ID}`;
  return{engine_id:ARENA_ID,starting_equity:num(arena.starting_equity,1000),cash,equity:eq,realized_pnl:realized,trade_count:trades,win_count:wins,loss_count:losses,positions:marks,
    recent_events:recentEvents.slice(0,20),mode:'AGGRESSIVE_ARENA',source:'SHARED_MAIN_SCAN',no_extra_full_scan:true,monster_override:true};
}
async function run(sql:any){const state=await loadState(sql),now=Date.now();if(!state.enabled||now>=Date.parse(state.run_until)){const scan={status:'WINDOW_COMPLETE',policy_version:POLICY_VERSION,engine_version:ENGINE_VERSION};await sql`update public.brian_dip_multiasset_state set enabled=false,updated_at=now(),last_scan=${sql.json(scan)} where engine_id=${ENGINE_ID}`;return{status:'WINDOW_COMPLETE',engine_version:ENGINE_VERSION};}const arena=await loadArenaState(sql,state),radar=await loadUniverse(state),symbols=[...new Set([...Object.keys(state.positions||{}),...Object.keys(arena.positions||{}),...radar.rows.map((x:Candidate)=>x.symbol)])].slice(0,MAX_DEEP_SCAN+MAX_POSITIONS+3),pairs=await Promise.all(symbols.map(async s=>{try{return[s,await loadMarket(s,radar.books.get(s))] as const;}catch(e){return[s,{error:e instanceof Error?e.message:String(e)}] as const;}})),markets=new Map<string,Market>(),marketErrors:J={};for(const[s,v]of pairs){if('error'in v)marketErrors[s]=v.error;else markets.set(s,v);}const positions={...(state.positions||{})},cooldowns={...(state.cooldowns||{})};let cash=num(state.cash),realized=num(state.realized_pnl),trades=num(state.trade_count),wins=num(state.win_count),losses=num(state.loss_count),lossStreak=num((state.last_scan as any)?.loss_streak),lastLossAt=num((state.last_scan as any)?.last_loss_at);const actions:any[]=[];
...
const breadth=marketBreadth(markets),
  mode=riskMode({...state,realized_pnl:realized,win_count:wins,loss_count:losses,last_scan:{...state.last_scan,loss_streak:lossStreak,last_loss_at:lastLossAt}} as State,equity(cash,positions,markets),radar.stale),
  evals:any[]=[],ready:any[]=[],openCandidates:any[]=[],arenaPool:any[]=[],
...
for(const c of radar.rows){
  ...
  const ev=evaluate(c,m,mode,breadth),open=!!positions[c.symbol],guard:any=cooldowns[c.symbol],reentry=reentryContext(guard,m,ev,now);
  ...
  const lateCore=ev.coreRaw&&(m.pullbackPct<.10||ev.forecast.ret1_bps>95||ev.forecast.ret3_bps>180)&&ev.forecast.expected_15m_bps<95,
    ...
    style=reentryReady?(reentry.type==='RUNNER_RECLAIM'?'RUNNER_RECLAIM':reentry.type==='STOP_RECLAIM'?'REENTRY_STOP':reentry.type==='NEW_WAVE_RESET'?'NEW_WAVE_RESET':'REENTRY_RESET'):
    ...
    rec:any={symbol:c.symbol,price:m.mid,radar_score:c.radar_score,radar_lane:c.lane||'UNKNOWN',signal_score:ev.score,opportunity_score:ev.opp,opportunity_tier:ev.tier,
      ...
      spread_bps:m.spreadBps,forecast_utility:ev.forecastUtility,brain_utility:ev.brainUtility,ready:isReady,entry_style:style,reentry_type:reentry.type,reentry_reason:reentry.reason,reentry_attempt:num(guard?.reentry_count),
  ...
  evals.push(rec);arenaPool.push({c,m,ev,rec});if(open)openCandidates.push({c,m,ev,rec});if(isReady)ready.push({c,m,ev,style,reentry,rec});
}
...
ready.sort((a,b)=>(b.style==='EMERGENCY_SCOUT'?1:0)-(a.style==='EMERGENCY_SCOUT'?1:0)||b.ev.brainUtility-a.ev.brainUtility||b.ev.capitalScore-a.ev.capitalScore||b.ev.opp-a.ev.opp);
for(const x of ready){
  ...
  if(x.reentry.type==='RUNNER_RECLAIM')profile={gross:Math.min(.025,Math.max(.015,profile.gross*.35)),risk:Math.min(.0012,Math.max(.0007,profile.risk*.35))};
  else if(x.reentry.type==='STOP_RECLAIM'||x.reentry.type==='NEW_WAVE_RESET')profile={gross:profile.gross*.55,risk:profile.risk*.55};
  ...
  const baseStopDist=Math.min(x.m.ask*.020,Math.max(x.m.atr*1.35,x.m.ask*.0070)),
    ...
    resetEntry=x.reentry.type==='STOP_RECLAIM'||x.reentry.type==='PROFIT_RESET'||x.reentry.type==='NEW_WAVE_RESET'||x.reentry.type==='RUNNER_RECLAIM',
  ...
  const isReentry=['STOP_RECLAIM','PROFIT_RESET','NEW_WAVE_RESET','RUNNER_RECLAIM'].includes(x.reentry.type),
    ...
    entryReason=x.style==='EMERGENCY_SCOUT'?'V880_EMERGENCY_SCOUT':x.style==='SURGE_SCOUT'?'V880_SURGE_SCOUT':
      x.reentry.type==='RUNNER_RECLAIM'?'V884_RUNNER_RECLAIM':x.reentry.type==='STOP_RECLAIM'?'V880_STOP_RECLAIM':x.reentry.type==='PROFIT_RESET'?'V880_PROFIT_RESET':x.reentry.type==='NEW_WAVE_RESET'?'V880_NEW_WAVE_RESET':
  ...
  const p:Position={symbol:x.c.symbol,entry,qty,opened_at:iso(),stop:entry-stopDist,target:entry+harvestDist,trail:null,max_price:entry,cost_basis:costBasis,
    ...
    entry_forecast_utility:x.ev.forecastUtility,entry_brain_utility:x.ev.brainUtility,last_forecast_utility:x.ev.forecastUtility,peak_forecast_utility:x.ev.forecastUtility,thesis_decay_streak:0,last_thesis_at:now,
  ...
  const row={observed_at:iso(),symbol:x.c.symbol,action:'BUY',price:entry,qty,notional:gross,pnl:null,reason:p.entry_reason,metadata:{
    ...
    risk_mode:mode.name,capital_score:x.ev.capitalScore,explosion_score:x.ev.explosionScore,forecast_utility:x.ev.forecastUtility,brain_utility:x.ev.brainUtility,continuous_conviction_sizing:true,capital_fraction:p.capital_fraction,
...
const arenaSnapshot=await processArena(sql,arena,arenaPool,markets,breadth,now);
