...
async function processArena(sql:any,arena:any,pool:any[],markets:Map<string,Market>,breadth:Breadth,now:number){
  const positions:{[k:string]:any}={...(arena.positions||{})};
  ...
  const allCandidates=pool.filter(x=>!positions[x.c.symbol]).map(x=>{
    const f=x.ev.forecast,g=x.ev.gates||{},barKey=num(x.m?.lastBarCloseTime),
      safe=Boolean(g.spread&&g.day_move&&g.forecast_consistency&&g.regime_guard&&g.shock_memory&&g.post_pump_reset&&g.long_extension&&g.economics),
      baseQualified=safe&&x.ev.explosionScore>=adaptiveExplosion&&x.ev.brainUtility>=.80&&x.ev.forecastUtility>=.72&&x.ev.opp>=.68&&
        f.continuation>=(breadth.riskOff?.66:.64)&&x.ev.net>=30&&f.expected_15m_bps>=20&&f.expected_30m_bps>=35,
      eliteException=safe&&x.ev.explosionScore>=.78&&x.ev.brainUtility>=.84&&x.ev.opp>=.72&&f.continuation>=.64&&x.ev.net>=70&&f.expected_30m_bps>=80,
      qualified=Boolean(baseQualified||eliteException),
    ...
    candidateMemory[x.c.symbol]={streak,last_seen:now,last_evidence_bar_close:barKey,arena_score:arenaScore,explosion_score:x.ev.explosionScore,
      ...
      qualified,rebound_test:reboundTest};
    candidateStreaks[x.c.symbol]=streak;
    return{...x,arenaScore,arenaQualified:qualified,arenaStreak:streak,arenaNewEvidenceBar:newEvidenceBar,arenaReboundTest:reboundTest};
  ...
  if(!monsterOverride){
    for(const x of candidates){
      ...
      const exConv=clip((x.ev.explosionScore-adaptiveExplosion)/Math.max(.06,.90-adaptiveExplosion)),
        brainConv=clip((x.ev.brainUtility-.78)/.15),forecastConv=clip((x.ev.forecastUtility-.70)/.22),
        oppConv=clip((x.ev.opp-.66)/.20),contConv=clip((x.ev.forecast.continuation-.62)/.24),
        evidenceConv=clip(exConv*.55+brainConv*.15+forecastConv*.12+oppConv*.10+contConv*.08),
        firstFrac=.22+.48*evidenceConv,
        secondAllowed=count===0||x.arenaScore>=Math.max(.77,(candidates[0]?.arenaScore||0)-.07),
        secondFrac=.18+.30*evidenceConv,
  ...
  const arenaWatch=allCandidates.slice(0,8).map(x=>({symbol:x.c.symbol,qualified:x.arenaQualified,confirm_streak:x.arenaStreak,new_evidence_bar:x.arenaNewEvidenceBar,rebound_test:x.arenaReboundTest,
    arena_score:x.arenaScore,brain_utility:x.ev.brainUtility,forecast_utility:x.ev.forecastUtility,explosion_score:x.ev.explosionScore,opportunity_score:x.ev.opp,
  ...
  await sql`update public.brian_dip_multiasset_state set cash=${cash},realized_pnl=${realized},trade_count=${trades},win_count=${wins},loss_count=${losses},
    positions=${sql.json(positions)},last_scan=${sql.json(scan)},updated_at=now(),enabled=true,shadow_only=true,live_execution=false,run_until=${new Date(arena.run_until)}
