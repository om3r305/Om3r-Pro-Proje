...
const ENGINE_VERSION='V8.7.9';
const POLICY_VERSION='dip-v879-reentry-reset-pulse-20260920.1';
const RISK_ENGINE='V879_REENTRY_RESET_PULSE';
...
type Eval={coreRaw:boolean;winnerRaw:boolean;scoutRaw:boolean;hunterRaw:boolean;score:number;opp:number;cost:number;gross:number;net:number;tier:string;capitalScore:number;explosionScore:number;forecast:HorizonForecast;reason:string;gates:Record<string,boolean>};
...
async function loadUniverse(state:State){
  ...
  return{rows,books,source:'MULTI_RADAR_UNIVERSE',ageSec:0,stale:false,observedAt:iso(),universeCount:scored.length,laneCounts,prevMemory:alive,priceSnapshot,selectionPlan:{max_deep_scan:MAX_DEEP_SCAN,core_slots:CORE_SCAN_SLOTS,interrupt_slots:INTERRUPT_SLOTS,explorer_slots:EXPLORER_SLOTS,core_count:coreCount,interrupt_count:interruptCount,explorer_count:explorerCount}};
}
...
function evaluate(c:Candidate,m:Market,mode:any):Eval{const atrBps=m.atrPct*100,slip=Math.max(2,m.spreadBps/2),cost=2*FEE_BPS+m.spreadBps+2*slip,reqPull=Math.max(.45,m.atrPct*.60),reqBounce=Math.max(.12,m.atrPct*.13),struct=Math.max(0,(m.recentHigh-m.ask)/m.ask*10000),quality=clip(.58+(c.radar_score-.64)*.72+clip(c.momentum_score-.48,0,.52)*.18+m.forecast.continuation*.10,.54,1.02),gross=Math.min(Math.max(struct,atrBps*.42),atrBps*1.90)*quality,unc=Math.max(7,atrBps*.15,cost*.30),net=gross-cost-unc,score=clip(c.radar_score*.27+clip(m.pullbackPct/Math.max(reqPull*1.8,.58))*.16+clip(m.bouncePct/Math.max(reqBounce*2.8,.22))*.10+(m.recovery?.08:0)+(m.trendOk?.07:0)+m.forecast.continuation*.17+clip(net/120)*.12),opp=clip(score*.50+c.radar_score*.18+clip(c.momentum_score)*.06+m.forecast.continuation*.15+clip(net/120)*.09),explosionScore=clip(score*.28+opp*.24+m.forecast.continuation*.16+clip(net/140)*.16+clip(gross/Math.max(cost*2,1))*.08+clip(c.volatility_score)*.04+clip(c.momentum_score)*.04),impulse5=Math.max(180,atrBps*1.50),impulse3=Math.max(125,atrBps*1.05),impulse1=Math.max(85,atrBps*.75),f=m.forecast,rawConflict=(f.ret15_bps<-150&&f.expected_15m_bps>15)||(f.ret5_bps<-70&&f.ret3_bps<-45)||(f.ret3_bps<-80&&f.ret5_bps<20),regimeConflict=f.ret15_bps<-80&&f.ret30_bps<-100&&f.ret60_bps<-180&&f.trend_bps<-20,shockLimit=Math.max(240,atrBps*1.35),shockMemoryBlock=m.shock_up_15m_bps>=shockLimit&&m.shock_age_min<=12&&m.pullbackPct<Math.max(1.5,m.atrPct*.80),pullbackOverride=m.pullbackPct>=Math.max(1.5,m.atrPct*.85),long30=Math.max(500,atrBps*4.0),long60=Math.max(800,atrBps*6.5),gates={radar:c.radar_score>=(mode.name==='DEFENSIVE'?.72:.68),spread:m.spreadBps<=(mode.name==='DEFENSIVE'?10:14),day_move:Math.abs(c.price_change_pct)<=40,pullback:m.pullbackPct>=reqPull,bounce:m.bouncePct>=reqBounce,recovery:m.recovery,trend:m.trendOk,not_chasing:m.mid<=m.recentHigh*(1-Math.max(.0015,m.atrPct*.10/100)),forecast:f.continuation>=.60&&f.expected_15m_bps>=4&&f.expected_30m_bps>=0,forecast_consistency:!rawConflict,regime_guard:!regimeConflict,shock_memory:!shockMemoryBlock,impulse:f.ret5_bps<=impulse5&&f.ret3_bps<=impulse3&&f.ret1_bps<=impulse1,extension:m.bouncePct<=Math.max(3.0,m.atrPct*2.0),long_extension:(f.ret30_bps<=long30||pullbackOverride)&&(f.ret60_bps<=long60||pullbackOverride),economics:net>=Math.max(10,cost*.40)&&gross>=cost*1.60};const failed=Object.entries(gates).filter(([,v])=>!v).map(([k])=>k),allGates=failed.length===0,coreRaw=!mode.frozen&&score>=mode.minScore&&opp>=mode.minOpp&&allGates,winnerRaw=!mode.frozen&&!coreRaw&&allGates&&score>=Math.max(.79,mode.minScore-.01)&&opp>=Math.max(.73,mode.minOpp-.012)&&f.continuation>=.63&&f.expected_15m_bps>=8&&f.expected_30m_bps>=5&&net>=Math.max(18,cost*.55),critical=['spread','day_move','bounce','recovery','trend','forecast','forecast_consistency','regime_guard','shock_memory','extension','long_extension'],criticalOk=critical.every(k=>gates[k]),hunterAllowed=failed.every(k=>['radar','economics','pullback','not_chasing','impulse'].includes(k)),hunterRaw=!mode.frozen&&!coreRaw&&!winnerRaw&&criticalOk&&hunterAllowed&&score>=.80&&opp>=.76&&explosionScore>=.72&&f.continuation>=.72&&net>=14&&cost<=42&&gross>=cost*1.60&&(!failed.includes('radar')||score>=.84&&opp>=.80&&net>=22)&&(!failed.includes('economics')||net>=22)&&(!failed.includes('pullback')||f.expected_15m_bps>=18&&f.expected_30m_bps>=18)&&(!failed.includes('not_chasing')||f.expected_15m_bps>=25&&f.expected_30m_bps>=25)&&(!failed.includes('impulse')||score>=.84&&opp>=.80&&explosionScore>=.80&&f.continuation>=.76&&net>=28),scoutRaw=!mode.frozen&&!coreRaw&&!winnerRaw&&!hunterRaw&&((failed.length===1&&failed[0]==='economics'&&score>=.81&&opp>=.75&&f.continuation>=.67&&net>=5&&cost<=40&&gross>=cost*1.20)||(failed.length===0&&score>=.74&&opp>=.64&&explosionScore>=.62&&f.continuation>=.66&&net>=20&&cost<=38&&gross>=cost*1.45)),tier=tierOf(score,opp,net,f.continuation),capitalScore=clip(score*.34+opp*.30+f.continuation*.20+clip(net/100)*.16);return{coreRaw,winnerRaw,scoutRaw,hunterRaw,score,opp,cost,gross,net,tier,capitalScore,explosionScore,forecast:f,reason:coreRaw?'V873_CORE_CONFIRM':winnerRaw?'V873_WINNER_SCOUT_CONFIRM':mode.frozen?`RISK_FROZEN_${mode.reason}`:`WAIT_${failed.join('+')||'SCORE'}`,gates};}
function capitalProfile(mode:any,ev:Eval,style:string){let gross=mode.gross,risk=mode.risk;if(mode.name==='DEFENSIVE'){gross=ev.tier==='A+'?.060:ev.tier==='A'?.055:ev.tier==='B+'?.050:.045;risk=ev.tier==='A+'?.0032:ev.tier==='A'?.0030:ev.tier==='B+'?.0028:.0025;}else if(mode.name==='COLD'){gross=ev.tier==='A+'?.11:ev.tier==='A'?.09:ev.tier==='B+'?.075:.065;risk=ev.tier==='A+'?.0045:ev.tier==='A'?.0042:ev.tier==='B+'?.0038:.0035;}else if(mode.name==='NORMAL'){gross=ev.tier==='A+'?.16:ev.tier==='A'?.13:ev.tier==='B+'?.10:.09;risk=ev.tier==='A+'?.0060:ev.tier==='A'?.0055:ev.tier==='B+'?.0050:.0045;}if(style==='RECOVERY_SCOUT'||style==='EARLY_SCOUT'){gross*=.45;risk*=.45;}else if(style==='HUNTER'){gross*=.80;risk*=.80;}else if(style==='WINNER_SCOUT'){gross*=.52;risk*=.52;}if(ev.explosionScore>=.90)gross=Math.max(gross,mode.name==='DEFENSIVE'?.13:.16);else if(ev.explosionScore>=.86)gross=Math.max(gross,mode.name==='DEFENSIVE'?.10:.13);else if(ev.explosionScore>=.82)gross=Math.max(gross,mode.name==='DEFENSIVE'?.075:.10);return{gross:Math.min(.18,gross),risk:Math.min(.006,risk)};}

function reentryContext(guard:any,m:Market,ev:Eval,now:number){
  if(!guard||num(guard.exit_at)<=0||now-num(guard.exit_at)>90*60_000)return{active:false,eligible:false,type:'FRESH',reason:'FRESH',guard};
  ...
  const critical=Boolean(ev.gates.spread&&ev.gates.forecast_consistency&&ev.gates.regime_guard&&ev.gates.shock_memory&&ev.gates.extension&&ev.gates.long_extension);
  if(pnl<-.01&&reason==='STOP'){
    ...
    return{active:true,eligible,type:'STOP_RECLAIM',reason:eligible?'V879_STOP_RECLAIM':!reclaimed?'WAIT_STOP_RECLAIM':!noChase?'WAIT_REENTRY_NO_CHASE':'WAIT_REENTRY_THESIS',guard};
  }
  ...
  if(pnl>.01){
    ...
    return{active:true,eligible,type:'PROFIT_RESET',reason:eligible?'V879_PROFIT_RESET':!resetSeen?'WAIT_PROFIT_DIP':!reclaimed?'WAIT_PROFIT_RECLAIM':!noChase?'WAIT_PROFIT_NO_CHASE':bank<=0?'WAIT_PROFIT_BANK':'WAIT_PROFIT_THESIS',guard};
  }
...
function pulseOk(p:EntryPulse,reentryType:string){
  if(reentryType==='STOP_RECLAIM')return p.buy_ratio>=.50&&p.ret5_bps>=-12&&p.ret15_bps>=-25;
  if(reentryType==='PROFIT_RESET')return p.buy_ratio>=.52&&p.ret5_bps>=-8&&p.ret15_bps>=-18;
  return p.buy_ratio>=.44&&p.ret5_bps>=-25&&p.ret15_bps>=-45;
}
...
const mode=riskMode({...state,realized_pnl:realized,win_count:wins,loss_count:losses,last_scan:{...state.last_scan,loss_streak:lossStreak,last_loss_at:lastLossAt}} as State,equity(cash,positions,markets),radar.stale),evals:any[]=[],ready:any[]=[],prevCore=(((state.last_scan as any)?.core_streaks||{}) as Record<string,number>),prevWinner=(((state.last_scan as any)?.winner_streaks||{}) as Record<string,number>),prevHunter=(((state.last_scan as any)?.hunter_streaks||{}) as Record<string,number>),prevScout=(((state.last_scan as any)?.scout_streaks||{}) as Record<string,number>),coreStreaks:Record<string,number>={},winnerStreaks:Record<string,number>={},hunterStreaks:Record<string,number>={},scoutStreaks:Record<string,number>={};for(const c of radar.rows){
  const m=markets.get(c.symbol);if(!m)continue;
  const ev=evaluate(c,m,mode),open=!!positions[c.symbol],guard:any=cooldowns[c.symbol];
  const reentry=reentryContext(guard,m,ev,now);
  ...
  coreStreaks[c.symbol]=coreStreak;winnerStreaks[c.symbol]=winnerStreak;hunterStreaks[c.symbol]=hunterStreak;scoutStreaks[c.symbol]=scoutStreak;
  const lateCore=ev.coreRaw&&(m.pullbackPct<.10||ev.forecast.ret1_bps>95||ev.forecast.ret3_bps>180)&&ev.forecast.expected_15m_bps<95,
    ...
    normalReady=!reentry.active&&(coreReady||winnerReady||hunterReady||scoutReady),reentryReady=reentry.active&&reentry.eligible,
    isReady=!open&&(normalReady||reentryReady),
    style=reentryReady?(reentry.type==='STOP_RECLAIM'?'REENTRY_STOP':'REENTRY_RESET'):coreReady?'CORE':winnerReady?'WINNER_SCOUT':hunterReady?'HUNTER':scoutReady?'EARLY_SCOUT':'WAIT',
    reason=open?'POSITION_OPEN':reentry.active&&!reentry.eligible?reentry.reason:reentryReady?reentry.reason:coreReady?'V875_CORE_EDGE':winnerReady?'V875_WINNER_SCOUT':hunterReady?'V875_HUNTER_EDGE':scoutReady?'V875_EARLY_SCOUT':lateCore?'WAIT_LATE_ENTRY':ev.coreRaw?'WAIT_CORE_CONFIRM':ev.winnerRaw?'WAIT_WINNER_CONFIRM':ev.hunterRaw?'WAIT_HUNTER_CONFIRM':ev.scoutRaw?'WAIT_SCOUT_CONFIRM':ev.reason,
    rec:any={symbol:c.symbol,price:m.mid,radar_score:c.radar_score,radar_lane:c.lane||'UNKNOWN',signal_score:ev.score,opportunity_score:ev.opp,opportunity_tier:ev.tier,
      ...
      core_streak:coreStreak,winner_streak:winnerStreak,hunter_streak:hunterStreak,scout_streak:scoutStreak,reason,gates:ev.gates,policy_version:POLICY_VERSION,engine_version:ENGINE_VERSION,risk_mode:mode.name};
...
for(const x of ready){
  ...
  if(!pulseOk(pulse,x.reentry.type)){
    x.rec.ready=false;x.rec.reason='WAIT_ENTRY_PULSE';continue;
  }

  let profile=capitalProfile(mode,x.ev,x.style);
  if(x.reentry.type==='STOP_RECLAIM')profile={gross:profile.gross*.55,risk:profile.risk*.55};
  ...
  const baseStopDist=Math.min(x.m.ask*.020,Math.max(x.m.atr*1.35,x.m.ask*.0070)),
    ...
    stopDist=(x.reentry.type==='STOP_RECLAIM'||x.reentry.type==='PROFIT_RESET')?Math.min(x.m.ask*.025,Math.max(baseStopDist,structuralDist)):baseStopDist,
  ...
  const isReentry=x.reentry.type==='STOP_RECLAIM'||x.reentry.type==='PROFIT_RESET',
    attempt=isReentry?num(x.reentry.guard?.reentry_count)+1:0,bank=isReentry?num(x.reentry.guard?.wave_profit_bank):0,
    waveStarted=isReentry?num(x.reentry.guard?.wave_started_at,now):now,
    entryReason=x.reentry.type==='STOP_RECLAIM'?'V879_STOP_RECLAIM':x.reentry.type==='PROFIT_RESET'?'V879_PROFIT_RESET':x.style==='WINNER_SCOUT'?'V873_WINNER_SCOUT':x.style==='HUNTER'?'V873_HUNTER_EDGE':x.style==='RECOVERY_SCOUT'?'V873_RECOVERY_SCOUT':'V873_CORE_EDGE';
  ...
  const row={observed_at:iso(),symbol:x.c.symbol,action:'BUY',price:entry,qty,notional:gross,pnl:null,reason:p.entry_reason,metadata:{
    ...
    entry_guard_v872:true,profit_ratchet:true,entry_pulse:pulse,reentry_type:p.reentry_type,reentry_attempt:p.reentry_attempt,wave_profit_bank:p.wave_profit_bank,
