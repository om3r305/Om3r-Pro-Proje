...
const ENGINE_VERSION='V8.9.4';
const POLICY_VERSION='dip-v894-breadth-arena-memory-20260921.1';
const RISK_ENGINE='V894_BREADTH_ARENA_MEMORY';
...
async function processArena(sql:any,arena:any,pool:any[],markets:Map<string,Market>,breadth:Breadth,now:number){
  ...
  for(const [symbol,p] of Object.entries(positions)){
    ...
    const fu=forecastUtility(m),vote=arenaVote(m),entryU=num(p.entry_forecast_utility,fu),peakU=Math.max(num(p.peak_forecast_utility,entryU),fu),
      soft=(entryU-fu>.11||peakU-fu>.17)&&vote<0,streak=soft?Math.min(3,num(p.thesis_decay_streak)+1):Math.max(0,num(p.thesis_decay_streak)-1),
  ...
  const allCandidates=pool.filter(x=>!positions[x.c.symbol]).map(x=>{
    const f=x.ev.forecast,g=x.ev.gates||{},
      ...
      streak=qualified?Math.min(4,(prevFresh?num(prevMem.streak):0)+1):0,
      arenaScore=clip(x.ev.explosionScore*.48+x.ev.brainUtility*.23+x.ev.forecastUtility*.11+x.ev.opp*.10+f.continuation*.08);
    if(qualified)candidateMemory[x.c.symbol]={streak,last_seen:now,arena_score:arenaScore,explosion_score:x.ev.explosionScore};
    else delete candidateMemory[x.c.symbol];
    candidateStreaks[x.c.symbol]=streak;
    return{...x,arenaScore,arenaQualified:qualified,arenaStreak:streak};
  ...
  if(!monsterOverride){
    for(const x of candidates){
      ...
      const explosionConv=clip((x.ev.explosionScore-.78)/.10),brainConv=clip((x.ev.brainUtility-.78)/.14),
        firstFrac=.50+.20*(explosionConv*.72+brainConv*.28),
        secondAllowed=count===0||x.arenaScore>=Math.max(.79,(candidates[0]?.arenaScore||0)-.08),
        targetFrac=count===0?firstFrac:(secondAllowed?Math.min(.48,cash/equity*.995):0),
      ...
      const m=x.m,slip=Math.max(2,m.spreadBps/2),gross=budget/(1+FEE_BPS/10000),entry=m.ask*(1+slip/10000),qty=gross/entry,fee=gross*FEE_BPS/10000,costBasis=gross+fee,
        stopGap=Math.max(.05,m.atrPct*2.5/100),p:any={symbol:x.c.symbol,entry,qty,cost_basis:costBasis,opened_at:iso(),max_price:entry,disaster_stop:entry*(1-stopGap),
          ...
          entry_explosion_score:x.ev.explosionScore,allocation_fraction:gross/equity,slot_kind:'REGULAR',thesis_decay_streak:0};
      ...
      const ev={observed_at:iso(),symbol:p.symbol,action:'BUY',price:entry,qty,notional:gross,pnl:null,reason:count===0?'ARENA_EXPLOSION_TOP1':'ARENA_EXPLOSION_TOP2',metadata:{
        arena:true,arena_mode:'TOP2_CONCENTRATED',arena_score:clip(x.ev.brainUtility*.55+x.ev.forecastUtility*.25+x.ev.explosionScore*.20),brain_utility:x.ev.brainUtility,
        forecast_utility:x.ev.forecastUtility,explosion_score:x.ev.explosionScore,allocation_fraction:p.allocation_fraction,engine_version:ENGINE_VERSION,policy_version:POLICY_VERSION,shadow_only:true,live_execution:false}};
      await insertArenaEvent(sql,ev);actions.push(ev);recentEvents.unshift(ev);recentEvents.splice(20);
    }
  }
  ...
  const arenaWatch=allCandidates.slice(0,8).map(x=>({symbol:x.c.symbol,qualified:x.arenaQualified,confirm_streak:x.arenaStreak,arena_score:x.arenaScore,brain_utility:x.ev.brainUtility,forecast_utility:x.ev.forecastUtility,
    explosion_score:x.ev.explosionScore,opportunity_score:x.ev.opp,continuation:x.ev.forecast.continuation,forecast_net_bps:x.ev.net,expected_15m_bps:x.ev.forecast.expected_15m_bps,expected_30m_bps:x.ev.forecast.expected_30m_bps}));
  const scan={status:'RUNNING',mode:'AGGRESSIVE_ARENA',engine_version:ENGINE_VERSION,policy_version:POLICY_VERSION,source:'SHARED_MAIN_SCAN',heartbeat_at:iso(),
    ...
    adaptive_explosion_threshold:adaptiveExplosion,explosion_p90:p90,candidate_streaks:candidateStreaks,candidate_memory:candidateMemory,confirmation_cycles:2,candidate_memory_ttl_seconds:180,breadth,top1_target:'50-70%',top2_only_if_qualified:true,monster_target:'85%',ultra_monster_target:'99.5%',
