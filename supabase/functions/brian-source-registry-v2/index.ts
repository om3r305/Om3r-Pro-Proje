...
async function run(){
  ...
  const runId=await sha(`${COLLECTOR_ID}|${runStarted}|${results.length}|${ok}`);const receipt=await db.from("brian_collector_runs").insert({run_id:runId,collector_id:COLLECTOR_ID,started_at:runStarted,finished_at:new Date().toISOString(),status:degraded.length?"DEGRADED":"SUCCESS",observed_records:results.length,stored_records:results.length,degraded_sources:degraded.map(r=>`${r.endpoint_id}:${r.error}`).slice(0,24),error_class:null,error_message:null,evidence_class:EVIDENCE,shadow_only:true,live_execution:false,metadata:{source_arch_version:"V2",batch_size:BATCH,concurrency:CONCURRENCY,ok,eligible,due_total:due.length,decision_evidence_locked:true,fair_queue:"OLDEST_DUE_FIRST"}});if(receipt.error)throw receipt.error;
