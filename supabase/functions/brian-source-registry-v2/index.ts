...
function probeTimeout(endpoint:any){const configured=Number(endpoint?.metadata?.probe_timeout_ms);if(Number.isFinite(configured))return Math.max(5000,Math.min(25000,Math.trunc(configured)));if(endpoint.endpoint_id==="sec_edgar_current")return 20000;return 9000}
...
async function probe(endpoint:any){
  ...
  const rows=recent.data||[];const sampleCount=rows.length;const latestHealthy=Boolean(rows[0]?.reachable&&rows[0]?.parseable&&rows[0]?.origin_match);const healthySamples=rows.filter(row=>row.reachable&&row.parseable&&row.origin_match).length;
  const healthScore=clamp(sampleCount?healthySamples/sampleCount:0),authority=authorityPrior(endpoint.tier),lead=leadPrior(endpoint.tier),originality=originalityPrior(endpoint.tier),relevance=clamp(Number(endpoint.priority||50)/100),precision=.5,manipulation=clamp(Number(endpoint.manipulation_risk||.1));
  const composite=clamp(.30*authority+.15*lead+.15*originality+.15*relevance+.15*healthScore+.10*precision-.15*manipulation);
  const researchEligible=sampleCount>=2&&healthySamples>=2&&composite>=.72&&["T0_RAW_TELEMETRY","T1_OFFICIAL_PRIMARY","T2_INSTITUTIONAL"].includes(endpoint.tier);
  const scoreId=await sha(`${endpoint.endpoint_id}|${observedAt}|${composite.toFixed(6)}|${healthySamples}|${sampleCount}`);const reasons=[`healthy_samples=${healthySamples}/${sampleCount}`,`latest_healthy=${latestHealthy}`,`composite=${composite.toFixed(3)}`,researchEligible?"research_eligible":"research_not_yet_eligible","decision_evidence_locked"];
  const scoreInsert=await db.from("brian_source_scores_v2").insert({score_id:scoreId,endpoint_id:endpoint.endpoint_id,assessed_at:observedAt,authority_score:authority,lead_time_score:lead,originality_score:originality,market_relevance_score:relevance,manipulation_risk:manipulation,historical_precision_score:precision,health_score:healthScore,composite_score:composite,sample_count:sampleCount,eligible_for_research:researchEligible,eligible_for_decision_evidence:false,reasons,metadata:{source_arch_version:"V2",healthy_samples:healthySamples,latest_healthy:latestHealthy,decision_evidence_locked:true,runtime:RUNTIME},shadow_only:true,live_execution:false});if(scoreInsert.error)throw scoreInsert.error;
  const lifecycle=researchEligible?"ACTIVE":(latestHealthy||healthySamples>0?"VERIFYING":"DEGRADED");const update=await db.from("brian_source_endpoints_v2").update({lifecycle_state:lifecycle,updated_at:new Date().toISOString()}).eq("endpoint_id",endpoint.endpoint_id);if(update.error)throw update.error;
  return {endpoint_id:endpoint.endpoint_id,reachable,parseable:pars,origin_match:origin,http_status:status,latency_ms:latency,healthy_samples:healthySamples,sample_count:sampleCount,latest_healthy:latestHealthy,composite_score:Number(composite.toFixed(4)),research_eligible:researchEligible,lifecycle,error:errorMessage};
}
...
async function run(){
  ...
  const runId=await sha(`${COLLECTOR_ID}|${runStarted}|${results.length}|${ok}`);const receipt=await db.from("brian_collector_runs").insert({run_id:runId,collector_id:COLLECTOR_ID,started_at:runStarted,finished_at:new Date().toISOString(),status:degraded.length?"DEGRADED":"SUCCESS",observed_records:results.length,stored_records:results.length,degraded_sources:degraded.map(r=>`${r.endpoint_id}:${r.error}`).slice(0,24),error_class:null,error_message:null,evidence_class:EVIDENCE,shadow_only:true,live_execution:false,metadata:{source_arch_version:"V2",runtime:RUNTIME,batch_size:BATCH,concurrency:CONCURRENCY,ok,eligible,due_total:due.length,decision_evidence_locked:true,fair_queue:"OLDEST_DUE_FIRST",health_policy:"TWO_OF_THREE_QUORUM"}});if(receipt.error)throw receipt.error;
