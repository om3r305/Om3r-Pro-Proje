// Brian DIP V8.4.2 Long-only Stateful contracts. SHADOW ONLY.
// V8.3 and prior V8.4/V8.4.1 evidence remain historical baselines.
...
export const METRIC_VERSION = "target-before-invalidation-v842.1" as const;
export const RESOLVER_VERSION = "dip-v842-path-20260911.1" as const;
export const ENTRY_GUARD_VERSION = "brian-live-entry-quality-v2" as const;
export const TARGET_PLANNER_VERSION = "brian-dynamic-advisory-target-v2" as const;
export const EXECUTION_MODEL_VERSION = "stateful-long-exit-v842.1" as const;
export const COST_MODEL_VERSION = "FILL_FORWARD_V842_1" as const;
...
export const STRATEGY_MANIFEST = Object.freeze({
  ...
  decision_authority:"BRIAN_LONG_ONLY_STATEFUL",
...
export function validateSession(config:J):void {
  if(config.release_id!==RELEASE_ID||config.logic_hash!==LOGIC_HASH||config.strategy_manifest_hash!==STRATEGY_MANIFEST_HASH||config.calibration_family_id!==CALIBRATION_FAMILY_ID||config.db_contract_version!==DB_CONTRACT_VERSION)throw Error("V842_RELEASE_CONTRACT_MISMATCH");
  if(config.engine_version!==ENGINE_VERSION||config.policy_version!==POLICY_VERSION||config.decision_revision!==DECISION_REVISION||config.metric_version!==METRIC_VERSION)throw Error("V842_VERSION_CONTRACT_MISMATCH");
  if(JSON.stringify(config.symbols)!==JSON.stringify([SYMBOL]))throw Error("V842_SYMBOL_CONTRACT_MISMATCH");
  if(config.shadow_only!==true||config.live_execution!==false||config.browser_execution!==false||config.server_authoritative!==true)throw Error("V842_INVALID_SHADOW_CONTRACT");
  if(config.allow_shadow_short!==false||n(config.max_shadow_leverage,0)!==MAX_SHADOW_LEVERAGE)throw Error("V842_INVALID_LONG_ONLY_CONTRACT");
  if(!["OBSERVE","SHADOW_PAPER"].includes(String(config.execution_mode)))throw Error("V842_INVALID_EXECUTION_MODE");
  if(config.decision_authority!=="BRIAN")throw Error("V842_AUTHORITY_CONTRACT_MISMATCH");
}