...
export const WORLD_ENGINEERING_BRIDGE_VERSION = "brian.world-engineering-bridge.v1";
...
export interface ResearchInputs { gaps:GapSignal[]; challenger:ChallengerSignal|null; outcomes:OutcomeSignal[]; reliability:ReliabilitySignal[]; worldSources?:WorldSourceSignal[]; observedAt:string; }
...
export function generateResearchHypotheses(inputs:ResearchInputs):HypothesisCandidate[]{
  ...
  const trustedWorldSources=(inputs.worldSources??[])
    ...
    .filter(source=>Number.isFinite(source.trustScore)&&source.trustScore>=0.72)
  ...
  for(const {source,host} of trustedWorldSources){
    ...
    addUnique(out,{
      ...
      evidenceRefs:[...source.evidenceRefs,`world_source:${source.sourceId}`,`world_source_uri:${source.canonicalUri}`].slice(0,40),
      counterEvidenceRefs:[],
      measurableSuccessCriteria:[
