...
function applyClose(rt:Runtime,p:Position,exit:number,recordedAt:number,reason:string,extra:J={}):J{
  if(p.side!=="LONG")throw Error("V842_SHORT_POSITION_FORBIDDEN");
  const feeClose=exit*p.qty*p.fee_bps/10000,gross=(exit-p.entry)*p.qty;
  const funding=p.funding_accrued??0;if(!Number.isFinite(funding))throw Error("V842_INVALID_FUNDING");
...
export function closeOnStop(rt:Runtime,barrier:number,recordedAt:number,extra:J={}):J|null{
  const p=rt.pos;if(!p)return null;if(p.side!=="LONG")throw Error("V842_SHORT_POSITION_FORBIDDEN");
  if(!(barrier>0)||!Number.isFinite(barrier))throw Error("V842_INVALID_STOP_BARRIER");
...
export function closeOnBrianSell(rt:Runtime,market:Market,recordedAt:number,thesis:J,sellVotes:number):J|null{
  ...
  if(action!=="SELL"||sellVotes<2)return null;
  const confidence=Number(thesis.authority_confidence||0);
  return applyClose(rt,p,executableLongExit(p,market),recordedAt,"BRIAN_STATEFUL_SELL",{authority_action:action,authority_confidence:confidence,authority_reason:thesis.authority_reason,authority_scores:thesis.authority_scores,sell_votes:sellVotes,market_bid:market.book.bid,market_ask:market.book.ask,spread_applied_via_executable_side:true,execution_ambiguous_loss:false});
}
