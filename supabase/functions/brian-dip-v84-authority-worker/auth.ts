...
export async function withAuthorityLease<T>(db:SupabaseClient,work:(lease:Lease)=>Promise<T>):Promise<{contended:boolean;value?:T}>{
  const leaseKey="brian-dip-v84-authority-worker",owner=crypto.randomUUID(),seconds=55;
  const a=await db.rpc("brian_dip_v84_acquire_lease",{p_lease_key:leaseKey,p_owner_id:owner,p_lease_seconds:seconds});
  if(a.error)throw Error("V841_LEASE_READ_FAILED:"+a.error.message);
  const row=Array.isArray(a.data)?a.data[0]:a.data;if(!row||row.acquired!==true)return{contended:true};
  const generation=Number(row.lease_generation);if(!Number.isSafeInteger(generation)||generation<=0)throw Error("V841_INVALID_LEASE_GENERATION");
  let lost=false,finished=false;
  const timer=setInterval(async()=>{try{const r=await db.rpc("brian_dip_v84_renew_lease",{p_lease_key:leaseKey,p_owner_id:owner,p_lease_generation:generation,p_lease_seconds:seconds});if(r.error||r.data!==true)lost=true;}catch{lost=true;}if(lost||finished)clearInterval(timer);},18000);
  const assertOwned=()=>{if(lost)throw Error("V841_LEASE_LOST");};
