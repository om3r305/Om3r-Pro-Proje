...
export async function assertReleaseSealed(db:SupabaseClient):Promise<void>{
  if(String(LOGIC_HASH)==="UNSEALED_GITHUB_ONLY")throw Error("V841_RELEASE_NOT_SEALED_IN_SOURCE");
...
async function readRuntime(db:SupabaseClient,sessionId:string):Promise<{runtime:Runtime;state_version:number}>{
  const q=await db.from("brian_dip_v84_runtime").select("runtime,state_version").eq("session_id",sessionId).maybeSingle();
  if(q.error)throw Error("V841_RUNTIME_READ_FAILED:"+q.error.message);if(!q.data)throw Error("V841_STATE_MISSING_RECONCILE");
  const rt=q.data.runtime as Runtime;
  if(!rt||![rt.start,rt.cash,rt.realized,rt.trades,rt.wins,rt.losses,rt.marketCursor,Number(q.data.state_version)].every(x=>typeof x==="number"&&Number.isFinite(x))||rt.start<=0||rt.cash<0||!Object.hasOwn(rt,"pos"))throw Error("V841_INVALID_RUNTIME_RECONCILE");
...
async function episodeEntered(db:SupabaseClient,episode:string):Promise<boolean>{
  ...
  if(q.error)throw Error("V841_EPISODE_READ_FAILED:"+q.error.message);return!!q.data?.length;
}
async function loadExecutionCalibration(db:SupabaseClient,setup:string,direction:string,regime:string):Promise<ExecutionCalibration>{
...
function paperMark(pos:NonNullable<Runtime["pos"]>,market:Awaited<ReturnType<typeof getMarket>>|null){if(!market)return pos.market_price;const friction=pos.expected_exit_slippage_bps/10000;return pos.side==="LONG"?market.book.bid*(1-friction):market.book.ask*(1+friction);}
...
async function recordCandidateTelemetry(db:SupabaseClient,sid:string,c:CandidateResult,at:number,market:Awaited<ReturnType<typeof getMarket>>){
  try{
    ...
    const evaluationId=await hash(["v841-eval",sid,c.occurrence,at].join("|"));
    await db.from("brian_dip_v84_candidate_evaluations").insert({evaluation_id:evaluationId,session_id:sid,occurrence_id:c.occurrence||null,episode_id:c.episode||null,evaluated_at:new Date(at).toISOString(),release_id:RELEASE_ID,strategy_manifest_hash:STRATEGY_MANIFEST_HASH,candidate_set:c.candidateEvaluations,first_blocking_veto:c.firstBlockingVeto,veto_stage:c.vetoStage,spread_bps:market.book.spreadBps,atr_1m:Number(s?.s1?.atr||0)||null,atr_5m:Number(s?.s5?.atr||0)||null,atr_15m:Number(s?.s15?.atr||0)||null,eval_offset_ms_from_seal:Math.max(0,at-latestSeal),payload:{decision_authority:"BRIAN",authority_action:c.thesis.authority_action,authority_confidence:c.thesis.authority_confidence,authority_allocation:c.thesis.authority_allocation,soft_evidence:c.thesis.soft_evidence,signal_at:new Date(signalAt).toISOString(),ofi_at_first_eval:market.flowFast.ofi,book_pressure:market.book.pressure,market_available_at:new Date(market.availableAt).toISOString(),market_data_contract_version:MARKET_DATA_CONTRACT_VERSION}});
...
export async function runWorker(db:SupabaseClient,lease:Lease,fetcher:typeof fetch=fetch):Promise<J>{
  ...
  if(rt.pos){
    ...
    try{const path=await pricePath(start,due,Date.now(),p.market_price||p.entry,fetcher);const {evaluatePath}=await import("../_shared/dip_v84_contract.ts");const resolution=evaluatePath({direction:p.side==="LONG"?"UP":"DOWN",target:p.target,stop:p.stop,start,due,now:Date.now(),end:path.end,entry:p.market_price||p.entry,segments:path.segments});if(resolution.reason==="INDETERMINATE")throw Error("V841_PRICE_PATH_INDETERMINATE");const funding=await positionFunding(p,resolution.eventAt??resolution.checkedUntil,fetcher);p.funding_accrued=funding.cashflow;p.funding_settlements=funding.settlements;p.checked_until=resolution.checkedUntil;p.market_price=resolution.price;const closed=closePosition(rt,resolution,Date.now());if(closed)events.push(closed);}catch(e){pathError=e instanceof Error?e.message:String(e);}
  }
  ...
  if(market&&c.canEnter&&entryDecision&&!rt.pos&&!events.length&&!pathError&&!stale&&sess.event_kind==="START"&&c.target&&c.inv&&c.size){
  ...
  const state={start:rt.start,cfg:{...cfg,symbols:[SYMBOL],trade_notional:tradeNotional},symbols:{[SYMBOL]:{symbol:SYMBOL,last:market?.book.mid??rt.pos?.market_price??null,price:market?.book.mid??rt.pos?.market_price??null,pos:rt.pos,armed:false,lastAction:rt.pos?rt.pos.side:"WATCH",thesis:frozenRuntimeThesis}},v84:rt,serverRuntime:{authoritative:true,decision_authority:"BRIAN",release_id:RELEASE_ID,logic_hash:LOGIC_HASH,strategy_manifest_hash:STRATEGY_MANIFEST_HASH,calibration_family_id:CALIBRATION_FAMILY_ID,policy_version:POLICY_VERSION,metric_version:METRIC_VERSION,generated_at:recordedAt,state_version:loaded.state_version+1,browser_executor_disabled:true,shadow_only:true,live_execution:false,dual_direction:true,max_shadow_leverage:1,risk_promotion_enabled:false,decision_cadence_seconds:15,market_source:market?.source||"BINANCE_USDM_PERP",status:pathError?"RECONCILIATION_REQUIRED":marketError?"DATA_UNAVAILABLE":"OK"}};
  const snapshot={snapshot_id:`v841-hour-${sid}-${hour}`,session_id:sid,observed_at:recordedAt,cash:rt.cash,equity,realized_pnl:rt.realized,unrealized_pnl:unrealized,trade_count:rt.trades,win_count:rt.wins,loss_count:rt.losses,state};
  for(const e of events)e.transition_id=`v841-${String(e.event_kind).toLowerCase()}-${e.occurrence_id}`;
  ...
  const committed=await db.rpc("brian_dip_v84_commit",{p_session_id:sid,p_expected_version:loaded.state_version,p_owner_id:lease.owner,p_lease_generation:lease.generation,p_commit_id:commitId,p_runtime:rt,p_snapshot:snapshot,p_decision:decision,p_events:events});if(committed.error)throw Error("V841_COMMIT_FAILED:"+committed.error.message);
  if(market)await recordCandidateTelemetry(db,sid,c,at,market);
  return{status:pathError?"RECONCILIATION_REQUIRED":marketError?"DATA_UNAVAILABLE":"OK",market_error:marketError,session_id:sid,release_id:RELEASE_ID,state_version:loaded.state_version+1,symbol:SYMBOL,thesis:frozenRuntimeThesis,position:rt.pos,equity,realized:rt.realized,events:events.map(e=>e.event_kind),shadow_only:true,live_execution:false,browser_execution:false};
}
