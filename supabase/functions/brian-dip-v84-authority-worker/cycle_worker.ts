...
import { authorityCandidate } from "./cycle.ts";
...
type Memory={sellVotes:number;lastExitAt:number|null;lastExitPrice:number|null;lastExitReason:string|null;lastTargetRaiseAt:number|null;lastPeakPrice:number|null;postExitLow:number|null;rebaseReady:boolean};
type Runtime844=Runtime&{v842?:Memory};

function memory(rt:Runtime844):Memory{
  ...
  const m=rt.v842 as Memory;
  if(!Object.hasOwn(m,"lastPeakPrice"))m.lastPeakPrice=null;
  if(!Object.hasOwn(m,"postExitLow"))m.postExitLow=null;
  if(!Object.hasOwn(m,"rebaseReady"))m.rebaseReady=false;
  return m;
}

export async function assertReleaseSealed(db:SupabaseClient):Promise<void>{
  if(String(LOGIC_HASH)==="UNSEALED_GITHUB_ONLY")throw Error("V844_RELEASE_NOT_SEALED_IN_SOURCE");
  ...
  if(q.error||!q.data)throw Error("V844_RELEASE_REGISTRY_UNAVAILABLE");
  if(q.data.status!=="SEALED"||q.data.logic_hash!==LOGIC_HASH||q.data.strategy_manifest_hash!==STRATEGY_MANIFEST_HASH||q.data.calibration_family_id!==CALIBRATION_FAMILY_ID)throw Error("V844_RELEASE_REGISTRY_MISMATCH");
  if(q.data.manifest?.shadow_only!==true||q.data.manifest?.live_execution!==false||q.data.manifest?.browser_execution!==false||q.data.manifest?.short_entries!==false||Number(q.data.manifest?.max_shadow_leverage)!==1)throw Error("V844_RELEASE_SAFETY_MISMATCH");
}

async function readRuntime(db:SupabaseClient,sessionId:string):Promise<{runtime:Runtime844;state_version:number}>{
  const q=await db.from("brian_dip_v84_runtime").select("runtime,state_version").eq("session_id",sessionId).maybeSingle();
  if(q.error)throw Error("V844_RUNTIME_READ_FAILED:"+q.error.message);if(!q.data)throw Error("V844_STATE_MISSING_RECONCILE");
  const rt=q.data.runtime as Runtime844;
  if(!rt||![rt.start,rt.cash,rt.realized,rt.trades,rt.wins,rt.losses,rt.marketCursor,Number(q.data.state_version)].every(x=>typeof x==="number"&&Number.isFinite(x))||rt.start<=0||rt.cash<0||!Object.hasOwn(rt,"pos"))throw Error("V844_INVALID_RUNTIME_RECONCILE");
  if(rt.pos?.side==="SHORT")throw Error("V844_SHORT_POSITION_FORBIDDEN");memory(rt);
...
async function episodeEntered(db:SupabaseClient,episode:string):Promise<boolean>{
  ...
  if(q.error)throw Error("V844_EPISODE_READ_FAILED:"+q.error.message);return!!q.data?.length;
}
...
function rememberExit(rt:Runtime844,e:J|null){
  if(!e)return;const m=memory(rt),exit=Number(e.exit_price||e.price||0)||null,md=(e.metadata??{}) as J,scores=(md.authority_scores??{}) as J,peak=Number(scores.peak_bid||0);
  m.sellVotes=0;m.lastExitAt=Date.now();m.lastExitPrice=exit;m.lastExitReason=String(md.exit_reason||"SELL");m.lastPeakPrice=Number.isFinite(peak)&&peak>0?Math.max(peak,exit??0):exit;m.postExitLow=exit;m.rebaseReady=false;
}
...
async function latestAuthoritySession(db:SupabaseClient):Promise<{latest:any;start:any}|null>{
  ...
  if(startQ.error)throw Error("V844_START_READ_FAILED:"+startQ.error.message);if(!startQ.data)return null;
  ...
  if(q.error)throw Error("V844_SESSION_READ_FAILED:"+q.error.message);if(!q.data)throw Error("V844_SESSION_EVENT_MISSING");
...
export async function runWorker(db:SupabaseClient,lease:Lease,fetcher:typeof fetch=fetch):Promise<J>{
  ...
  const sid=String(startSess.session_id),startingEquity=Number(startSess.starting_equity||0),tradeNotional=Math.min(Number(startSess.trade_notional||startingEquity||0),startingEquity);if(!(tradeNotional>0))throw Error("V844_INVALID_TRADE_NOTIONAL");
  ...
  // Only hard structural invalidation auto-closes. Forecast targets are advisory and harvest is stateful.
  if(rt.pos){
  ...
  if(decision){
    const existing=await db.from("brian_dip_v84_decisions").select("occurrence_id,episode_id,setup,direction,target_price,invalidation_price").eq("occurrence_id",c.occurrence).maybeSingle();if(existing.error)throw Error("V844_DECISION_READ_FAILED:"+existing.error.message);
  ...
  const snapshot={snapshot_id:`v844-hour-${sid}-${hour}`,session_id:sid,observed_at:recordedAt,cash:rt.cash,equity,realized_pnl:rt.realized,unrealized_pnl:unrealized,trade_count:rt.trades,win_count:rt.wins,loss_count:rt.losses,state};
  for(const e of events)e.transition_id=`v844-${String(e.event_kind).toLowerCase()}-${e.occurrence_id}-${Date.now()}`;
  ...
  const committed=await db.rpc("brian_dip_v84_commit",{p_session_id:sid,p_expected_version:loaded.state_version,p_owner_id:lease.owner,p_lease_generation:lease.generation,p_commit_id:commitId,p_runtime:rt,p_snapshot:snapshot,p_decision:decision,p_events:events});if(committed.error)throw Error("V844_COMMIT_FAILED:"+committed.error.message);
