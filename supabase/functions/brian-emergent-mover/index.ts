...
async function fetchJson(
  ...
  try {
    ...
    return { payload: await response.json(), observedAt: new Date().toISOString(), degraded: false };
  } catch (error) {
...
function marketRows(
  ...
  for (const symbol of eligibleSymbols) {
    ...
    try {
      ...
      if (
        !(lastPrice > 0) || quoteVolume < 0 || !Number.isInteger(trades) || trades < 0 ||
        low < 0 || high < low
      ) {
      ...
      if (book) {
        try {
          ...
          if (bid > 0 && ask >= bid) spreadBps = 10_000 * (ask - bid) / ((bid + ask) / 2);
        } catch {
          // Optional top-of-book context may degrade per symbol; never fabricate a spread.
          spreadBps = null;
        }
      }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const lease = await withCollectorLease(supabase, COLLECTOR_ID, LEASE_SECONDS, async () => {
      // Provider-scoped baseline: future second venues cannot contaminate this Binance scout.
      ...
      // Provider-scoped universe lineage for the same reason.
      ...
      if (!universe.data?.snapshot_id || !universe.data?.observed_at) {
        throw new Error("no provider-matched universe snapshot available");
      }
      ...
      if (!coverage.complete) {
        ...
        // the next baseline. Raw provider truth above is intentionally retained for diagnosis.
        return jsonResponse({
      ...
      if (latestFrame.data?.state) previousFrame = parseEmergentMoverFrame(latestFrame.data.state);
