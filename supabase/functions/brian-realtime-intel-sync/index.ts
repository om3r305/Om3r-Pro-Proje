...
const VERSION="brian.realtime-intel-sync.v3-durable-delivery";
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const q=await db.rpc("brian_claim_intel_delivery",{p_token:token});
    if(q.error)throw q.error;
    ...
    if(Number(body.accepted)!==events.length)throw new Error("CORE_PARTIAL_ACCEPTANCE");
    const ack=await db.rpc("brian_ack_intel_delivery",{p_token:token,p_ids:events.map(e=>String(e.event_id))});
    if(ack.error)throw ack.error;
    if(Number(ack.data)!==events.length)throw new Error("DELIVERY_ACK_MISMATCH");
    ...
    return out({status:"SUCCESS",version:VERSION,delivery_mode:"DURABLE_ACK",scanned:events.length,core_accepted:Number(body.accepted??0),core_stored:stored,shadow_only:true,live_execution:false});
  }catch(e){
