...
const PROFIT_COOLDOWN_MS = 20 * 60_000;
...
type ReentryGuard = {
  ...
  peak_price:number; atr_pct:number;
};
type State = {
  ...
  positions:Record<string,Position>; cooldowns:Record<string,number|ReentryGuard>; last_scan:J;
...
function ema(values:number[],period:number):number{const k=2/(period+1);let out=values[0]??0;for(let i=1;i<values.length;i++)out=values[i]*k+out*(1-k);return out;}
function atr(bars:Bar[],period=14):number{if(bars.length<period+2)return 0;const xs:number[]=[];for(let i=1;i<bars.length;i++){const b=bars[i],p=bars[i-1].c;xs.push(Math.max(b.h-b.l,Math.abs(b.h-p),Math.abs(b.l-p)));}return xs.slice(-period).reduce((a,b)=>a+b,0)/period;}
async function loadMarket(symbol:string):Promise<Market>{
  ...
  const recent20=bars.slice(-20),recent12=bars.slice(-12),recentHigh=Math.max(...recent20.map(b=>b.h)),recentLow=Math.min(...recent12.map(b=>b.l));
...
async function acquireLease():Promise<{owner:string;generation:number}|null>{
  ...
  if(q.error)throw Error("LEASE:"+q.error.message);const row=Array.isArray(q.data)?q.data[0]:q.data;if(!row?.acquired)return null;return{owner,generation:Number(row.lease_generation)};
}
async function releaseLease(lease:{owner:string;generation:number}|null){if(!lease)return;try{await db.rpc("brian_dip_v84_release_lease",{p_lease_key:"brian-dip-multiasset-worker-v1",p_owner_id:lease.owner,p_lease_generation:lease.generation});}catch{}}
...
function readGuard(raw:unknown):ReentryGuard|null{
  ...
  const r=raw as J,until=num(r.until);if(!(until>0))return null;
  return{until,reason:String(r.reason||"UNKNOWN"),exit_price:num(r.exit_price),exit_at:num(r.exit_at,until-BASE_COOLDOWN_MS),pnl:num(r.pnl),peak_price:num(r.peak_price),atr_pct:num(r.atr_pct)};
}
function reentryCheck(raw:unknown,c:Candidate,m:Market,ev:EntryEval,now:number):GuardCheck{
  const g=readGuard(raw);if(!g)return{blocked:false,reason:"",clear:raw!=null,metrics:{}};
  const age=Math.max(0,now-g.exit_at),base={guard_reason:g.reason,guard_age_sec:Math.round(age/1000),cooldown_left_sec:Math.max(0,Math.round((g.until-now)/1000)),last_exit_price:g.exit_price,last_exit_pnl:g.pnl};
  if(now<g.until)return{blocked:true,reason:`COOLDOWN_${g.reason}`,clear:false,metrics:base};
  if(age>GUARD_MAX_AGE_MS||g.reason==="LEGACY")return{blocked:false,reason:"",clear:true,metrics:{...base,guard_reset:"AGE_OR_LEGACY"}};
  if(g.reason==="TARGET"){
    const needPull=Math.max(.75,m.atrPct*1.05,g.atr_pct*.90),needBounce=Math.max(.22,m.atrPct*.18);
    const belowExit=g.exit_price>0?m.mid<=g.exit_price*(1-Math.max(.0025,m.atrPct*.18/100)):true;
    const reset=m.pullbackPct>=needPull&&m.bouncePct>=needBounce&&belowExit&&m.recovery&&m.trendOk&&ev.score>=.72;
    return reset?{blocked:false,reason:"",clear:true,metrics:{...base,guard_reset:"NEW_DIP_AFTER_TARGET",need_pullback_pct:needPull,need_bounce_pct:needBounce}}:{blocked:true,reason:"TARGET_LEG_LOCK",clear:false,metrics:{...base,need_pullback_pct:needPull,need_bounce_pct:needBounce,below_exit:belowExit}};
  }
...
async function run():Promise<J>{
  ...
  let cash=num(state.cash),realized=num(state.realized_pnl),tradeCount=num(state.trade_count),wins=num(state.win_count),losses=num(state.loss_count);
  const actions:J[]=[];
