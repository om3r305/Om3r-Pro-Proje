...
const ACTION_GATE_HORIZON_SECONDS=900;
...
async function candidateExperiments(){
  ...
  return(expQ.data??[]).filter(row=>kinds.get(String(row.hypothesis_id))==="ACTION_GATE").slice(0,10);
}
...
async function loadActionGateEvidence(observedAt:string){
  ...
  const [labelsQ,outcomesQ]=await Promise.all([
    ...
    db.from("brian_alpha_decision_outcomes")
      .select("decision_id,observed_at,horizon_seconds,direction_adjusted_return,classification,metadata,resolved_at")
      .eq("horizon_seconds",ACTION_GATE_HORIZON_SECONDS).gte("resolved_at",since).order("observed_at",{ascending:true}).limit(12000),
  ]);
  if(labelsQ.error)throw new Error(`challenger_labels:${labelsQ.error.message}`);
  if(outcomesQ.error)throw new Error(`outcomes:${outcomesQ.error.message}`);
  const labels:ChallengerDecisionLabel[]=[];
  const openIds=new Set<string>();
  for(const row of labelsQ.data??[]){
    const canonical=String(row.canonical_action);if(canonical!=="OPEN_LONG"&&canonical!=="OPEN_SHORT")continue;
    const decisionId=String(row.decision_id);openIds.add(decisionId);labels.push({decisionId,challengerAction:String(row.challenger_action)});
  }
  const outcomes:ProspectiveOutcomePoint[]=[];
  for(const row of outcomesQ.data??[]){
    const decisionId=String(row.decision_id);if(!openIds.has(decisionId))continue;
    const directionAdjustedReturn=Number(row.direction_adjusted_return);const cost=Number(meta(row.metadata).estimated_round_trip_cost_bps);
...
async function persistMeasurement(experiment:Record<string,unknown>,observedAt:string){
  const evidence=await loadActionGateEvidence(observedAt);
  const measured=measureActionGateExperiment(evidence.outcomes,evidence.labels);
  ...
  for(const [role,metric] of [["CONTROL",measured.control],["CHALLENGER",measured.challenger]] as const){
    ...
    rows.push({
      ...
      metric_payload:{lab_version:EVOLUTION_LAB_VERSION,horizon_seconds:ACTION_GATE_HORIZON_SECONDS,measurement_kind:"PROSPECTIVE_ACTION_GATE",lineage:measured.lineage,control_version:experiment.control_version,challenger_version:experiment.challenger_version},
      evidence_refs:[`brian_alpha_calibration_challenger:7d`,`brian_alpha_decision_outcomes:${ACTION_GATE_HORIZON_SECONDS}s:7d`],
  ...
  return{experiment_id:experiment.experiment_id,control:measured.control,challenger:measured.challenger,lineage:measured.lineage,stored:rows.length};
}
