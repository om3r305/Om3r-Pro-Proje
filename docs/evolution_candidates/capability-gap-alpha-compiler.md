...
report. Equal-provider/equal-completion conflicts are detected across the
accepted input envelope before the decision row limit is applied.
...
freshness, failures, lease accounting, invalid counts, blockers, or truncation
state. Exact failure identities are deduplicated by provider, row, and failure
id.
