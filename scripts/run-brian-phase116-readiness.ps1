...
if ($missing.Count -gt 0) {
  [Console]::Error.WriteLine("Phase116 blocked: missing scoped backend secret variables: " + ($missing -join ", "))
  exit 30
}
...
python -m brian2026.phase116_crypto_shadow_readiness_entrypoint --policy "config/brian-shadow-machine-policy-v1.json" --runtime-id "brian-shadow-main"
exit $LASTEXITCODE
