...
function v4ClearRuntimeStates(){for(const s of Object.keys(states))delete states[s];}
...
restore=function(d){
  ...
  const cfg={...session.config,engine_version:DIP_EXPERT_V4,max_shadow_leverage:1};v4ClearRuntimeStates();book={start:Number(session.starting_equity),cash:Number(session.starting_equity),realized:0,trades:0,wins:0,losses:0,cfg};
