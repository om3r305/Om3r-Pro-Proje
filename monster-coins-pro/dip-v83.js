...
function renderKpis(){
  ...
  $('kpiPnlMeta').textContent='fee + slippage + spread; yeni kapanışlarda gerçekleşen funding';$('kpiOpenMeta').textContent=pos?`${pos.side} · ${num(pos.leverage,1)}x · notional ${money(pos.notional)}`:'Pozisyon yok · risk ≤ %0.5/trade';$('kpiWinMeta').textContent=`${wins} win / ${losses} loss`;$('kpiTradesMeta').textContent='kapalı round trips';$('kpiEngineMeta').textContent=`USD-M PERP · ${num(runtime()?.decision_cadence_seconds,60)} sn karar`;
}

function eventKind(e){return String(e?.event_kind||'—');}
function renderLog(){
  const rows=model.events||[];const host=$('eventRows');
  if(!rows.length){host.innerHTML='<tr><td colspan="10">Bu V8.3 session’da henüz LONG / SHORT açılış-kapanış olayı yok.</td></tr>';return;}
