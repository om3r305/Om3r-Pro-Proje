...
(function(){
 ...
 const roomActions={meeting:()=>$('openMeeting')?.click(),accounting:()=>window.openTreasuryAccounting?.(),anatomy:()=>window.openBrianAnatomy?.(),engineering:()=>window.openBrianEngineeringRoom?.()};
 shell.addEventListener('click',e=>{const b=e.target.closest('[data-ops-room]');if(b){const id=b.dataset.opsRoom;const available=id==='meeting'?!!$('openMeeting'):typeof window[{accounting:'openTreasuryAccounting',anatomy:'openBrianAnatomy',engineering:'openBrianEngineeringRoom'}[id]]==='function';if(available){$('opsRoomFeedback').textContent='';roomActions[id]()}else $('opsRoomFeedback').textContent='Oda henüz hazırlanıyor. Biraz sonra tekrar deneyebilirsin.';}
 ...
 $('opsEquity').textContent=money(v.equity);const count=hb?.control?.treasury?.open_positions;$('opsPositions').textContent=v.positions??(Number.isInteger(count)&&count>=0?count:'—');$('opsTreasuryTime').textContent=`${v.treasuryFresh?'Güncel kasa':'Son kayıt · güncellik doğrulanmadı'} · ${stamp(hb?.control?.treasury?.observed_at)}`;
