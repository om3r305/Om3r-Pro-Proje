...
(function(){
  ...
  let socket=null,retry=null,retries=0,lastRest=0,lastPaint=0,lastTrade=null,lastKline=null,lastMark=null,connectedAt=0,paintTimer=null;
  const seen={};
  ...
  function labels(){
    ...
    const lag=live?Date.now()-lastTrade.eventAt:null;const receivedAge=live?Math.max(0,Date.now()-lastTrade.at):null;
    if(bar){bar.textContent=`${live?`Son veri alınalı ${receivedAge} ms${lag<0?' · cihaz/borsa saat farkı var':''}`:'Anlık bağlantı yok; REST görüntüsü'} · Brian kararı ${age} sn önce (${Number(sr?.decision_cadence_seconds)||15} sn döngü)`;bar.style.color=live?'#91a2b8':'#f0b90b';}
  }
  ...
  function paint(){if(document.hidden||paintTimer)return;const delay=Math.max(0,100-(Date.now()-lastPaint));paintTimer=setTimeout(()=>{paintTimer=null;lastPaint=Date.now();renderChart();},delay);}
  function connect(){
    ...
    ws.onmessage=function(ev){
      ...
      if(d.e==='aggTrade'&&Number(d.p)>0&&Number.isFinite(Number(d.p))){lastTrade={price:Number(d.p),eventAt,at};if(model.chart)model.chart.last_price=lastTrade.price;if(!document.hidden&&$('lastPrice'))$('lastPrice').textContent=px(lastTrade.price);}
    ...
    ws.onclose=function(){if(socket!==ws)return;socket=null;labels();{retry=setTimeout(()=>{retry=null;connect();},Math.min(30000,1000*2**Math.min(retries++,5)));}};
    ws.onerror=function(){ws.close();};
  }
  function suspend(){clearTimeout(retry);retry=null;const ws=socket;socket=null;if(ws)ws.close();}
  document.addEventListener('visibilitychange',()=>{if(!document.hidden){lastRest=0;loadChart();connect();paint();}});
  window.addEventListener('pagehide',suspend);
  window.addEventListener('pageshow',connect);
  setInterval(()=>{if(!document.hidden)labels();if(socket&&Date.now()-Math.max(connectedAt,lastTrade?.at||0)>20000)socket.close();},1000);
  connect();
})();
