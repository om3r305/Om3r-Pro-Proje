...
function v4HandleWsMessage(m,venue){
  const sym=String(m.s||'').toUpperCase();if(!sym)return;lastWs=v4Now();
  if(m.e==='aggTrade'){v4TapePush(venue==='USDM_PERP'?v4TapePerp:v4TapeSpot,sym,m);if(venue==='SPOT')tick(sym,Number(m.p));}
  else if(m.e==='kline'&&venue==='SPOT'){v4UpdateBar(sym,m.k.i,m.k);if(sym===selected)draw();}
  ...
  const now=v4Now();if(now-v4RenderAt>450){v4RenderAt=now;renderRadar();renderKpi();if(sym===selected)draw();}
}
function v4DispatchWsPacket(raw,venue){
  const packet=JSON.parse(raw),m=packet?.data||packet;if(!m||typeof m!=='object')return;
  if(!m.s&&typeof packet?.stream==='string'){
    const streamSymbol=packet.stream.split('@')[0];if(streamSymbol)m.s=streamSymbol.toUpperCase();
  }
  v4HandleWsMessage(m,venue);
}
...
connect=function(){
  ...
  v4SpotWs.onmessage=e=>{if(gen!==v4WsGeneration)return;try{v4DispatchWsPacket(e.data,'SPOT')}catch{}};
  v4SpotWs.onerror=()=>{try{v4SpotWs.close()}catch{}};v4SpotWs.onclose=()=>{if(gen===v4WsGeneration)v4ScheduleReconnect()};
  if(perp.length){v4PerpWs=new WebSocket(`wss://fstream.binance.com/stream?streams=${perp.join('/')}`);v4PerpWs.onmessage=e=>{if(gen!==v4WsGeneration)return;try{v4DispatchWsPacket(e.data,'USDM_PERP')}catch{}};v4PerpWs.onerror=()=>{};v4PerpWs.onclose=()=>{};}
};
