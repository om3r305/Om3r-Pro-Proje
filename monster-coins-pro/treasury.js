...
function render(data){
  ...
  $('trCash').textContent=usd(s.cash_usd);$('trReserve').textContent=`Nakit rezerv ${pct(s.cash_reserve_pct)} · politika ≥ 30%`;
  $('trDeployment').textContent=usd(s.deployment_usd);$('trDeploymentPct').textContent=`Deployment ${pct(s.deployment_pct)} · politika ≤ 70%`;
  $('trPositionsCount').textContent=String(s.open_positions??0);$('trCosts').textContent=`Kümülatif maliyet ${usd(s.cumulative_costs_usd)}`;

  $('trPositions').innerHTML=positions.map(p=>{const dir=Number(p.direction)>0?'LONG':'SHORT';const tone=Number(p.direction)>0?'tr-pos-long':'tr-pos-short';return`<div class="tr-row"><div class="tr-title ${tone}">${esc(p.assetId)} · ${dir} ${pill(usd(p.capitalUsd))}</div><div class="tr-meta">Giriş ${Number(p.entryPrice).toLocaleString('tr-TR')} · açılış ${clock(p.openedAt)}<br>Entry edge ${bps(p.entryExpectedNetEdgeBps)} · son edge ${bps(p.latestExpectedNetEdgeBps)} · maliyet ${bps(p.roundTripCostBps)}<br>Kaynak karar ${esc(p.sourceDecisionId)}</div></div>`;}).join('')||'<div class="tr-row">Açık sermaye pozisyonu yok. Treasury nakitte.</div>';

  $('trGateDetail').innerHTML=`<div class="tr-row"><div class="tr-title">${gate?pill('AUTHORIZED','ok'):pill('CLOSED','warn')}</div><div class="tr-meta">${esc(s.promotion_gate_reason||'—')}<br>Evidence: ${esc(s.promotion_gate_ref||'—')}<br>Gate kapanırsa yeni allocation durur ve mevcut SHADOW allocation flat edilir.</div></div>${(snap.blocked_reasons||[]).map(r=>`<div class="tr-row"><div class="tr-title">${pill('BLOCK','warn')} ${esc(r)}</div></div>`).join('')}`;
