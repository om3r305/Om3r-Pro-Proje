...
(function(){
  ...
  function showContext(){
    ...
    host.innerHTML=`<div class="title">${esc(heading)}</div><p class="note" style="margin:8px 0;line-height:1.6">${esc(explanation)}</p><div class="thesis-levels"><span>Hedef mesafesi <b>${pct(t.target_distance_bps)}</b></span><span>Modellenen toplam maliyet <b>${pct(t.cost_bps)}</b></span><span>Bağımsız kalibrasyon örneği <b>${Math.max(0,num(t.calibration_samples))}</b></span></div><p class="note" style="margin:8px 0;overflow-wrap:anywhere"><b>Seçili hedefin geçmişi:</b> ${esc(history)}</p><small class="note">Ham yapı puanı başarı olasılığı değildir. Seviye geçmişi yalnız açıklamadır; kırılmış seviye otomatik olarak geçersiz sayılmaz.</small>`;
  }
