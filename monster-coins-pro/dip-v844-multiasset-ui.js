...
(()=>{
  ...
  let latest=null,selected='',chart={symbol:'',candles:[],last:0,source:'WAIT',ws:null,reconnect:null,seq:0},positionTicks=new Map(),summaryQueued=false,summaryApplying=false;
  ...
  function candidateSymbols(){
    ...
    for(const p of latest?.positions||[])push(p.symbol,'open');for(const e of latest?.recent_events||[])push(e.symbol,'event');for(const e of latestUniqueEvaluations(latest?.recent_evaluations||[]))push(e.symbol,'watch');return out.slice(0,12);
  }
  ...
  function drawChart(){const cv=el('multiDipCanvas'),wrap=el('multiDipChartWrap');if(!cv||!wrap)return;const ctx=cv.getContext('2d'),dpr=window.devicePixelRatio||1,w=Math.max(300,wrap.clientWidth),h=Math.max(260,wrap.clientHeight);cv.width=Math.round(w*dpr);cv.height=Math.round(h*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);ctx.fillStyle='#07101a';ctx.fillRect(0,0,w,h);const rows=chart.candles.slice(-180);if(!selected||selected==='ETHUSDT'){ctx.fillStyle='#91a2b8';ctx.font='13px system-ui';ctx.fillText('ETH için aşağıdaki V8.4.4 çekirdek grafiğini kullan.',18,34);return;}if(rows.length<2){ctx.fillStyle='#91a2b8';ctx.font='13px system-ui';ctx.fillText('Binance Spot 1s mumları yükleniyor…',18,34);return;}const lev=planLevels(selected),levels=[lev.entry,lev.stop,lev.target,lev.trail,chart.last].filter(v=>n(v)>0),vals=rows.flatMap(c=>[c.l,c.h]).concat(levels);let lo=Math.min(...vals),hi=Math.max(...vals),pad=(hi-lo)*.09||hi*.001||.001;lo-=pad;hi+=pad;const L=9,R=78,T=18,B=28,pw=w-L-R,ph=h-T-B,cw=pw/rows.length,y=v=>T+(hi-v)/(hi-lo)*ph;for(let i=0;i<=4;i++){const yy=T+ph*i/4,v=hi-(hi-lo)*i/4;ctx.strokeStyle='#152436';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(L,yy);ctx.lineTo(w-R,yy);ctx.stroke();ctx.fillStyle='#718198';ctx.font='9px system-ui';ctx.fillText(price(v),w-R+6,yy+3);}rows.forEach((c,i)=>{const x=L+cw*i+cw/2,up=c.c>=c.o,col=up?'#12d996':'#ff5068',body=Math.max(1,Math.abs(y(c.o)-y(c.c))),top=Math.min(y(c.o),y(c.c)),ww=Math.max(1,Math.min(5,cw*.72));ctx.strokeStyle=col;ctx.fillStyle=col;ctx.beginPath();ctx.moveTo(x,y(c.h));ctx.lineTo(x,y(c.l));ctx.stroke();ctx.fillRect(x-ww/2,top,ww,body);});drawLine(ctx,y,L,R,w,'ENTRY',lev.entry,'#62a9ff');drawLine(ctx,y,L,R,w,'STOP',lev.stop,'#ff6278');drawLine(ctx,y,L,R,w,'HEDEF',lev.target,'#35f0ae');if(lev.trail>0)drawLine(ctx,y,L,R,w,'TRAIL',lev.trail,'#f3c969');drawLine(ctx,y,L,R,w,'CANLI',chart.last,'#e8f4ff');}

  async function seedChart(symbol,seq){let lastErr=null;for(const base of REST_HOSTS){try{const r=await fetch(`${base}/api/v3/klines?symbol=${encodeURIComponent(symbol)}&interval=1s&limit=180`,{cache:'no-store'});if(!r.ok)throw Error(`HTTP ${r.status}`);const raw=await r.json();if(!Array.isArray(raw)||raw.length<2)throw Error('MUM YOK');if(seq!==chart.seq)return;chart.candles=raw.map(a=>({t:n(a[0]),o:n(a[1]),h:n(a[2]),l:n(a[3]),c:n(a[4])})).filter(c=>c.t>0&&c.c>0);chart.last=chart.candles.at(-1)?.c||0;chart.source='REST + WS';drawChart();renderChartHead();return;}catch(e){lastErr=e;}}if(seq===chart.seq){chart.source='REST HATA';renderChartHead();drawChart();console.warn('multi dip chart seed',lastErr);}}
  ...
  function connectSocket(symbol,seq){stopSocket();if(!symbol||symbol==='ETHUSDT'||document.hidden)return;const url=`wss://stream.binance.com:9443/ws/${symbol.toLowerCase()}@kline_1s`;try{const ws=new WebSocket(url);chart.ws=ws;ws.onopen=()=>{if(seq!==chart.seq)return;chart.source='BINANCE 1s LIVE';renderChartHead();};ws.onmessage=ev=>{if(seq!==chart.seq)return;try{const d=JSON.parse(ev.data),k=d?.k;if(!k)return;const c={t:n(k.t),o:n(k.o),h:n(k.h),l:n(k.l),c:n(k.c)};if(!(c.t>0&&c.c>0))return;const last=chart.candles.at(-1);if(last?.t===c.t)chart.candles[chart.candles.length-1]=c;else{chart.candles.push(c);if(chart.candles.length>180)chart.candles.splice(0,chart.candles.length-180);}chart.last=c.c;positionTicks.set(symbol,{price:c.c,at:Date.now()});renderChartHead();renderPositionRows();drawChart();}catch{}};ws.onerror=()=>{chart.source='WS RETRY';renderChartHead();};ws.onclose=()=>{if(seq!==chart.seq||document.hidden)return;chart.reconnect=setTimeout(()=>connectSocket(symbol,seq),1800);};}catch{chart.reconnect=setTimeout(()=>connectSocket(symbol,seq),2200);}}
