...
(()=>{
  ...
  function ensure(){
    ...
    panel.innerHTML=`
      ...
      <div class="md-box md-arena">
        <div class="md-arena-head"><div><div class="md-arena-title">🧨 AGGRESSIVE ARENA · $1000 paralel SHADOW kasa</div><div class="md-meta">Aynı Brian gözleri ve aynı tarama. Normalde top-2 yoğunlaşma; gerçek MONSTER gelirse kârlı pozisyondan sermaye döndürüp çok daha yüksek konsantrasyon test edilir.</div></div><div id="mdArenaBadge" class="md-arena-badge">BAĞLANIYOR</div></div>
        ...
        <div class="md-note">Yük koruması: ikinci bir piyasa taraması açılmaz; Arena ana 32 derin taramayı paylaşır, ayrı evaluation geçmişi yazmaz. Yalnızca tek kompakt state satırı + gerçekleşen Arena BUY/SELL olayları tutulur.</div>
      </div>
  ...
  function renderArena(){
    const a=latest?.arena,badge=el('mdArenaBadge'),kpis=el('mdArenaKpis'),ph=el('mdArenaPositions'),eh=el('mdArenaEvents');
    ...
    if(badge){badge.textContent=running?'CANLI · SHARED SCAN':String(a.status||'WAIT');badge.style.color=running?'#35f0ae':'#f3c969';}
    ...
    if(ph)ph.innerHTML=positions.length?positions.map(p=>{const s=safeSymbol(p.symbol),alloc=n(p.allocation_fraction)*100,up=n(p.unrealized_pnl)>=0,kind=String(p.slot_kind||'REGULAR'),u=Math.round(n(p.brain_utility||p.forecast_utility)*100);return `<button class="md-row ${selected===s?'active':''}" data-symbol="${esc(s)}"><div><div class="md-main">${esc(s)} · ${esc(kind)}</div><div class="md-meta">Kasa %${alloc.toFixed(1)} · Entry ${price(p.entry)} · Mark ${price(p.mark)} · Brain ${u}/100</div></div><span class="md-side ${up?'pos':'neg'}">${money(p.unrealized_pnl)}</span></button>`;}).join(''):'<div class="md-row"><div><div class="md-main">Arena pozisyon bekliyor</div><div class="md-meta">En güçlü iki fırsat için sermaye hazır.</div></div><span class="md-side wait">0</span></div>';
