...
(()=>{
  ...
  function render(data){latest=data;ensure();const c=core(),linked=!data?.source_session_id||!c.session?.session_id||String(data.source_session_id)===String(c.session.session_id),state=el('multiDipState'),running=String(data?.status||'')==='RUNNING';if(state){state.textContent=running?`DIP CANLI · ${Math.round(n(data.age_seconds))} sn`:String(data?.status||'WAIT');state.style.color=running?'#35f0ae':'#f3c969';}setText(el('multiDipSession'),linked?`Tek session bağlı · ${String(data?.source_session_id||c.session?.session_id||'—')} · SHADOW ONLY · gerçek emir KAPALI`:`SESSION EŞLEŞMİYOR · ETH ${c.session?.session_id||'—'} · altcoin ${data?.source_session_id||'—'} · Yeni SHADOW session ile eşleştir.`);if(!selected){const first=(data?.positions||[])[0]?.symbol||(data?.recent_events||[])[0]?.symbol||(data?.recent_evaluations||[])[0]?.symbol;selected=safeSymbol(first)||'ETHUSDT';if(selected!=='ETHUSDT'){chart.symbol=selected;const seq=++chart.seq;seedChart(selected,seq);connectSocket(selected,seq);}}applyUnifiedSummary();renderTabs();renderLists();renderPlan();renderChartHead();drawChart();}
