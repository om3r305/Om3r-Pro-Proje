...
(()=>{
  ...
  function hb(){try{return typeof STABILITY!=='undefined'?STABILITY.heartbeat:null}catch(_e){return null}}
  ...
  function metrics(){
    ...
    const vals=[health,alpha,world].filter(Number.isFinite);
    const general=vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:null;
...
` ;document.head.appendChild(s)}
  function ensure(){
    ...
    const m=document.createElement('div');m.id='brianAnatomyLive';m.innerHTML=`<div class="bac"><header class="bac-head"><div class="bac-title"><span>BRIAN v2</span><b>Gelişim Anatomisi</b></div><div class="bac-act"><div class="bac-live" id="bacLive">CANLI</div><button class="bac-btn" id="bacRefresh">↻</button><button class="bac-btn" id="bacClose">×</button></div></header><div class="bac-kpis"><div class="bac-kpi"><span>GENEL GELİŞİM</span><b data-k="general">—</b></div><div class="bac-kpi"><span>SİSTEM SAĞLIĞI</span><b data-k="health">—</b></div><div class="bac-kpi"><span>ÖĞRENME</span><b data-k="learn">—</b></div><div class="bac-kpi"><span>KARAR KALİTESİ</span><b data-k="decision">—</b></div><div class="bac-kpi energy"><span>ENERJİ / CANLILIK</span><b data-k="energy">—</b></div></div><div class="bac-hero"><img src="/brian-anatomy-dashboard-reference.webp?v=20260913-clean2" alt="Brian gelişim anatomisi"><div class="bac-veil"></div><div class="bac-tag">BRIAN • GELİŞİM ANATOMİSİ</div></div><div class="bac-grid"><div class="bac-card"><span>BEYİN / ALPHA</span><b data-k="alpha">—</b></div><div class="bac-card"><span>DÜNYA / HAFIZA</span><b data-k="world">—</b></div><div class="bac-card"><span>COLLECTOR SAĞLIĞI</span><b data-k="health2">—</b></div><div class="bac-card"><span>SON HEARTBEAT</span><b data-k="time">—</b></div></div></div>`;document.body.appendChild(m);
    ...
    $('#bacRefresh').onclick=()=>{try{if(typeof refresh==='function')Promise.resolve(refresh()).finally(()=>setTimeout(render,250));else render()}catch(_e){render()}};
  }
