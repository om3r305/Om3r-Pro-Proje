...
(function(){
  ...
  loadChart=async function(){
    ...
    try{
      const r=await fetch(`${CHART_API}?view=spot1m&limit=180&_=${Date.now()}`,{cache:'no-store'});
      const d=await r.json().catch(()=>({}));
      if(!r.ok)throw new Error(safeText(d.error)||`HTTP ${r.status}`);
      if(!Array.isArray(d.candles)||d.candles.length<30)throw new Error('Binance Spot 1m mum verisi yetersiz');
  ...
  renderHealth=function(){
    ...
    if($('feedMeta'))$('feedMeta').textContent=workerFresh()&&chartFresh()?`Motor USD-M · Görsel Binance Spot 1m · karar ${num(sr?.decision_cadence_seconds,60)} sn`:(model.chartError||sr?.market_error||'heartbeat bekleniyor');
  ...
  renderChart=function(){
    ...
    if(!a.length){ctx.fillStyle='#91a2b8';ctx.font='14px system-ui';ctx.fillText(model.chartError||'Binance Spot 1m bekleniyor…',20,35);return;}
    ...
    const badge=$('chartSource');if(badge)badge.textContent='BINANCE SPOT · 1m';
    const title=document.querySelector('.chartHead .title');if(title)title.textContent='ETHUSDT · Binance Spot · 1m';
  ...
  render=function(){
    ...
    const pos=position();if(pos){for(const id of ['startBtn','restartBtn','pauseBtn']){const b=$(id);if(b){b.disabled=true;b.title='Açık pozisyon kapanana kadar session kontrolü kilitli.';}}}
  };
  ...
  window.addEventListener('load',()=>setInterval(()=>{
    ...
    bar.textContent=`Grafik Spot 1m ${marketAge} sn · Thesis ${thesisAge==null?'—':thesisAge+' sn'} · Worker ${workerAge==null?'—':workerAge+' sn'} · Motor karar 60 sn`;bar.style.color=workerAge!=null&&workerAge>135?'#ff6379':'#8ea1b8';
  },500));
})();
