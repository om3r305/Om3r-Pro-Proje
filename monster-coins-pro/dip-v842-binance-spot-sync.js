...
(()=>{
  ...
  function line(ctx,y,L,R,w,label,value,color,dash=[5,4],right=false){const v=n(value);if(!(v>0))return;const yy=y(v);if(!Number.isFinite(yy)||yy<18||yy>ctx.canvas.clientHeight-28)return;ctx.save();ctx.setLineDash(dash);ctx.strokeStyle=color;ctx.lineWidth=1.1;ctx.beginPath();ctx.moveTo(L,yy);ctx.lineTo(w-R,yy);ctx.stroke();ctx.setLineDash([]);ctx.font='700 10px system-ui';const text=`${label} ${fmt(v)}`,tw=ctx.measureText(text).width,x=right?Math.max(L+4,w-R-tw-7):L+6;ctx.fillStyle='rgba(5,10,18,.90)';ctx.fillRect(x-3,yy-15,tw+7,15);ctx.fillStyle=color;ctx.fillText(text,x,yy-4);ctx.restore();}
  ...
  renderChart=function(){
    ...
    const {lo,hi}=axisRange(rows),L=15,R=92,T=25,B=38,plotW=w-L-R,plotH=h-T-B,xw=plotW/rows.length,y=v=>T+(hi-v)/(hi-lo)*plotH;
    ...
    const t=tSafe()||{},pos=posSafe(),entry=pos?.entry??t.entry_price,target=pos?.target??t.target??t.target_price,stop=pos?.stop??t.invalidation??t.invalidation_price;
  ...
  function decorate(){
    ...
    const engine=el('kpiEngine');if(engine)engine.textContent='V8.4.2 LONG';
    const engineMeta=el('kpiEngineMeta');if(engineMeta)engineMeta.textContent='Brian USD-M SHADOW · ekran Binance Spot 1s · SHORT OFF';
    const ctx=el('decisionContext');if(ctx&&t){const scores=t.authority_scores||{},mem=(typeof model!=='undefined'?model.snapshot?.state?.v84?.v842:null)||{};const html=`<div class="thesis-main"><b>V8.4.2 LIVE SYNC</b><span>Mode <strong>LONG / SELL ONLY</strong></span><span>Brian action <strong>${String(t.authority_action||'WAIT')}</strong></span><span>Direction conf <strong>${Math.round(n(t.authority_confidence)*100)}%</strong></span><span>Entry quality <strong>${Math.round(n(t.authority_entry_quality)*100)}%</strong></span><span>Range <strong>${n(scores.range_position).toFixed(2)}</strong></span><span>Live mom <strong>${n(scores.momentum_atr).toFixed(2)} ATR</strong></span><span>Sell votes <strong>${n(mem.sellVotes)}</strong></span><span>SHORT <strong>OFF</strong></span></div>`;if(ctx.dataset.v842!==html){ctx.innerHTML=html;ctx.dataset.v842=html;}}
  }
