"""Static wiring/red-team contract for Brian ALPHA SHADOW runtime.

The decision compiler now lives in brian-realtime. Core is intentionally a fail-closed,
authenticated replication/sync surface for Treasury and dashboard consumers. These tests make
that split explicit so architecture migrations cannot appear as regressions or silently weaken
the ALPHA safety boundary.
"""
...
REALTIME_COMPILER = ROOT / "supabase/functions/brian-realtime-alpha-decision-compiler/index.ts"
CORE_SYNC = ROOT / "supabase/functions/brian-alpha-decision-compiler/index.ts"
...
def test_core_alpha_is_authenticated_sync_only_from_realtime():
    source = _text(CORE_SYNC)
    assert 'const VERSION = "brian.core-alpha-realtime-sync.v1"' in source
    assert 'const EXPORT_URL = "https://dliediwlldojkfjzlznm.supabase.co/functions/v1/brian-realtime-alpha-export"' in source
    assert '"x-brian-cron-key"' in source
...
def test_gdelt_is_discovery_only_at_realtime_runtime_and_pure_compiler_boundary():
    runtime = _text(REALTIME_COMPILER)
    pure = _text(DECISION)
...
def test_alpha_writers_require_application_level_auth():
    cron_auth = _text(CRON_AUTH)
    assert "x-brian-cron-key" in cron_auth
    assert "brian_dashboard_auth" in cron_auth
    assert "control-v3" in cron_auth
    assert "constantTimeEqual" in cron_auth
    for path in (AUDITOR, MACRO):
        text = _text(path)
        assert 'import { requireCronAuth } from "../_shared/cron_auth.ts";' in text
        assert "await requireCronAuth(req, supabase);" in text
        assert "UNAUTHORIZED" in text

    realtime_auth = _text(REALTIME_AUTH)
    runtime = _text(REALTIME_COMPILER)
    assert "x-brian-internal-key" in realtime_auth
    assert "constantTimeEqual" in realtime_auth
    assert 'import { requireRealtimeInternal } from "../_shared/realtime_internal_auth.ts";' in runtime
    assert "await requireRealtimeInternal(req);" in runtime
    assert "UNAUTHORIZED" in runtime

    core = _text(CORE_SYNC)
    assert '"x-brian-cron-key"' in core
    assert "EXPECTED_CRON_SHA256" in core
    assert "constantTimeEqual" in core
    assert "UNAUTHORIZED" in core
