...
HARDENING_MIGRATION=ROOT/"supabase"/"migrations"/"202609040017_brian_alpha_shadow_semantics.sql"
...
def _setup():
    ...
    try:
        with c.cursor() as cur:
            ...
            cur.execute(HARDENING_MIGRATION.read_text(encoding="utf-8"))
    finally:c.close()
...
def _insert_decision(cur,q,asset="crypto:BTCUSDT",observed_at="2026-09-04T12:00:00Z",action="OPEN_LONG",direction=1,reference_price=100):
    ...
    cur.execute("""
      ...
      values(%s,'test-v2',%s,%s,%s,%s,%s,0.3,2,
        array['intrabar_tape','derivatives_taker'],array[]::text[],array[]::text[],array[]::text[],%s,100,24,'fixture',true,false)
...
def test_shadow_position_book_reaffirms_then_flips_without_rebasing_entry():
    c=_connect();asset=f"crypto:STATE{uuid.uuid4().hex[:6].upper()}USDT"
    try:
        with c.cursor() as cur:
            cur.execute("set role service_role")
            q1=_valid_cost(cur,asset,"2026-09-04T14:00:00Z","BUY");d1=_insert_decision(cur,q1,asset,"2026-09-04T14:00:00Z","OPEN_LONG",1,100)
            q2=_valid_cost(cur,asset,"2026-09-04T14:01:00Z","BUY");d2=_insert_decision(cur,q2,asset,"2026-09-04T14:01:00Z","OPEN_LONG",1,102)
            q3=_valid_cost(cur,asset,"2026-09-04T14:02:00Z","SELL");d3=_insert_decision(cur,q3,asset,"2026-09-04T14:02:00Z","OPEN_SHORT",-1,99)
            cur.execute("""select decision_id,event_type,position_before,position_after,entry_price_after,realized_gross_bps from public.brian_alpha_shadow_position_events where asset_id=%s order by event_ts""",(asset,));rows=cur.fetchall()
            assert rows[0][:5]==(d1,"OPEN",0,1,100);assert rows[1][:5]==(d2,"REAFFIRM",1,1,100);assert rows[1][5] is None
            assert rows[2][:5]==(d3,"FLIP",1,-1,99);assert abs(rows[2][5]-(-100.0)) < 1e-9
            cur.execute("select position,entry_price,last_event_type from public.brian_alpha_shadow_position_book where asset_id=%s",(asset,));assert cur.fetchone()==(-1,99,"FLIP")
            cur.execute("reset role")
    finally:c.close()

def test_shadow_out_of_order_and_same_time_actions_do_not_rewrite_state():
    c=_connect();asset=f"crypto:ORDER{uuid.uuid4().hex[:6].upper()}USDT"
    try:
        with c.cursor() as cur:
            cur.execute("set role service_role")
            q1=_valid_cost(cur,asset,"2026-09-04T18:00:00Z","BUY");d1=_insert_decision(cur,q1,asset,"2026-09-04T18:00:00Z","OPEN_LONG",1,100)
            q2=_valid_cost(cur,asset,"2026-09-04T17:59:00Z","SELL");d2=_insert_decision(cur,q2,asset,"2026-09-04T17:59:00Z","OPEN_SHORT",-1,90)
            q3=_valid_cost(cur,asset,"2026-09-04T18:00:00Z","SELL");d3=_insert_decision(cur,q3,asset,"2026-09-04T18:00:00Z","OPEN_SHORT",-1,91)
            cur.execute("select decision_id,event_type,position_after,entry_price_after from public.brian_alpha_shadow_position_events where asset_id=%s order by event_ts",(asset,));rows=cur.fetchall()
            assert rows==[(d1,"OPEN",1,100)]
            cur.execute("select position,entry_price from public.brian_alpha_shadow_position_book where asset_id=%s",(asset,));assert cur.fetchone()==(1,100)
            cur.execute("select alpha_position_intent,(metadata->>'out_of_order_decision_ignored_for_position_intent')::boolean from public.brian_alpha_phase37_comparisons where decision_id=%s and phase37_policy_kind='NATIVE'",(d2,));assert cur.fetchone()==(0,True)
            cur.execute("select alpha_position_intent,(metadata->>'out_of_order_decision_ignored_for_position_intent')::boolean from public.brian_alpha_phase37_comparisons where decision_id=%s and phase37_policy_kind='NATIVE'",(d3,));assert cur.fetchone()==(1,True)
            cur.execute("reset role")
    finally:c.close()

def test_wait_holds_prior_shadow_position_in_phase37_comparator():
    c=_connect();asset=f"crypto:HOLD{uuid.uuid4().hex[:6].upper()}USDT";symbol=asset.split(":",1)[1];exp="phase37-prospective-live-20260903"
    try:
        with c.cursor() as cur:
            cur.execute("""insert into public.brian_live_shadow_ticks(tick_id,experiment_id,policy_kind,observed_at,target_weights) values(%s,%s,'NATIVE','2026-09-04T16:59:00Z',%s::jsonb),(%s,%s,'PROFIT','2026-09-04T16:59:00Z',%s::jsonb)""",(f"n-{uuid.uuid4().hex}",exp,f'{{"{symbol}":0.2}}',f"p-{uuid.uuid4().hex}",exp,f'{{"{symbol}":0.2}}'))
            cur.execute("set role service_role")
            q1=_valid_cost(cur,asset,"2026-09-04T17:00:00Z","BUY");_insert_decision(cur,q1,asset,"2026-09-04T17:00:00Z","OPEN_LONG",1,100)
            qw=_valid_cost(cur,asset,"2026-09-04T17:01:00Z","BUY");dw=_insert_decision(cur,qw,asset,"2026-09-04T17:01:00Z","WAIT",0,101)
            cur.execute("select count(*) from public.brian_alpha_shadow_position_events where asset_id=%s",(asset,));assert cur.fetchone()[0]==1
            cur.execute("select position,entry_price from public.brian_alpha_shadow_position_book where asset_id=%s",(asset,));assert cur.fetchone()==(1,100)
            cur.execute("select alpha_position_intent,relationship,metadata->>'wait_veto_semantics' from public.brian_alpha_phase37_comparisons where decision_id=%s and phase37_policy_kind='NATIVE'",(dw,));assert cur.fetchone()==(1,"AGREES","hold_prior_causal_shadow_position")
            cur.execute("reset role")
    finally:c.close()

def test_phase37_comparison_is_at_or_before_only_and_tracks_both_policies():
    c=_connect();asset=f"crypto:CMP{uuid.uuid4().hex[:6].upper()}USDT";symbol=asset.split(":",1)[1];exp="phase37-prospective-live-20260903"
    try:
        with c.cursor() as cur:
            cur.execute("""insert into public.brian_live_shadow_ticks(tick_id,experiment_id,policy_kind,observed_at,target_weights) values(%s,%s,'NATIVE','2026-09-04T15:59:00Z',%s::jsonb),(%s,%s,'NATIVE','2026-09-04T16:01:00Z',%s::jsonb),(%s,%s,'PROFIT','2026-09-04T15:58:00Z',%s::jsonb),(%s,%s,'PROFIT','2026-09-04T16:01:00Z',%s::jsonb)""",(f"nprev-{uuid.uuid4().hex}",exp,f'{{"{symbol}":0.2}}',f"nfut-{uuid.uuid4().hex}",exp,f'{{"{symbol}":-0.3}}',f"pprev-{uuid.uuid4().hex}",exp,f'{{"{symbol}":-0.1}}',f"pfut-{uuid.uuid4().hex}",exp,f'{{"{symbol}":0.3}}'))
            cur.execute("set role service_role");q=_valid_cost(cur,asset,"2026-09-04T16:00:00Z","BUY");d=_insert_decision(cur,q,asset,"2026-09-04T16:00:00Z","OPEN_LONG",1,100)
            cur.execute("select phase37_policy_kind,phase37_observed_at,phase37_direction,relationship from public.brian_alpha_phase37_comparisons where decision_id=%s order by phase37_policy_kind",(d,));rows=cur.fetchall();assert len(rows)==2
            native=next(r for r in rows if r[0]=="NATIVE");profit=next(r for r in rows if r[0]=="PROFIT")
            assert native[1].isoformat().startswith("2026-09-04T15:59:00");assert native[2:]==(1,"AGREES")
            assert profit[1].isoformat().startswith("2026-09-04T15:58:00");assert profit[2:]==(-1,"DISAGREES")
            cur.execute("reset role")
    finally:c.close()
